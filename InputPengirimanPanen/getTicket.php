...
<?php
		...
		if(isset($_POST['afdeling']) && isset($_POST['var_tgl']) && isset($_POST['deliv_ticket'])){
			...
			$sql_t_HP  = "select THP.ID_RENCANA as ID_RENCANA,
								 ...
								 NO_BCC,
								--(NVL(THK1.QTY,0)+NVL(THK2.QTY,0)+NVL(THK3.QTY,0)+NVL(THK4.QTY,0)+NVL(THK6.QTY,0)+NVL(THK15.QTY,0)) as JJG,  -- remarked by NBU 13.10.2015
								NVL (F_GET_HASIL_PANEN_BUNCH (SUBSTR(TDRP.ID_BA_AFD_BLOK,1,4), thp.no_rekap_bcc, thp.no_bcc, 'BUNCH_SEND'), 0) as JJG, 
								--THK5.QTY as BRD -- remarked by NBU 13.10.2015
						...
						/*left join T_HASILPANEN_KUALTAS THK1 on (THK1.ID_RENCANA = THP.ID_RENCANA and THK1.ID_BCC = THP.NO_BCC and THK1.ID_KUALITAS = '1') -- remarked by NBU 13.10.2015
						...
						where STATUS_BCC = 'RESTAN' and KODE_DELIVERY_TICKET like '$deliv_ticket%' and TDRP.ID_BA_AFD_BLOK like '$afdeling%' and TANGGAL_RENCANA <= TO_DATE('$tgl_panen', 'MM/DD/YYYY HH24:MI:SS')
			...
			while ($p=oci_fetch($result_t_HP)){
				...
				?>
				<a class='class_a'><li class='class_li' onclick='fill("<?php echo oci_result($result_t_HP, "NO_BCC") . " - " . oci_result($result_t_HP, "ID_RENCANA"); ?>")'><?php echo oci_result($result_t_HP, "TANGGAL_RENCANA") . " - " . oci_result($result_t_HP, "BLOK") . " - " . oci_result($result_t_HP, "NIK_PEMANEN") . " - " . oci_result($result_t_HP, "EMP_NAME")

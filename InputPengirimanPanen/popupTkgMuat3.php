...
<HTML>
<HEAD>
<TITLE>Daftar Tukang Muat</TITLE>
<SCRIPT LANGUAGE="JavaScript">
<!--
function pick(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputpengirimanpanen.nikTkgMuat3.value = symbol;
	window.close();
	}
}
function pickNAMA(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputpengirimanpanen.namaTkgMuat3.value = symbol;
	window.close();
	}
}
...
IF ($baris=="1"){
	$query = "select COUNT(TJ.ID_BA) as JUMLAH
                 ...
                 where AUTHORITY = '13' and ID_BA_AFD = '$afdeling'";
	...
	$query_emp = "select TJ.ID_BA as BA, 
                 ...
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '13' and ID_BA_AFD = '$afdeling' and INACTIVE_DATE is null";
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			$query_emp =   "Select TJ.ID_BA as BA, 
							...
							TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
						 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
						 where AUTHORITY = '13' and ID_BA_AFD = '$afdeling' and INACTIVE_DATE is null AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%')";
			$sql = oci_parse($con,$query_emp);
			oci_execute($sql, OCI_DEFAULT);
			$results=array();
...
ELSE{
	...
	$query = "select COUNT(TJ.ID_BA) as JUMLAH
				 from T_JOBAUTHORITY TJ 
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '13' AND (" . $id_ba . ")"; //added by NB 01.07.2014
	$sql1 = oci_parse($con,$query);
			oci_execute($sql1, OCI_DEFAULT);
	$numRow = oci_fetch_row($sql1);
	...
	$query_emp = "select TJ.ID_BA as BA, 
					...
					TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '13' AND (" . $id_ba . ") and INACTIVE_DATE is NULL
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			$query_emp =   "select TJ.ID_BA as BA, 
							...
							TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
						 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
						 where AUTHORITY = '13' AND (" . $id_ba . ") AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%') and INACTIVE_DATE is NULL

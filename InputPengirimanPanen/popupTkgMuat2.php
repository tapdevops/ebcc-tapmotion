...
function pick(symbol,row) {
    ...
    window.opener.document.inputpengirimanpanen.nikTkgMuat2.value = symbol;
	window.close();
	}
}
function pickNAMA(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputpengirimanpanen.namaTkgMuat2.value = symbol;
	window.close();
	}
}
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			$query_emp =   "Select TJ.ID_BA as BA, 
							...
							TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
						 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
						 where AUTHORITY = '13' and ID_BA_AFD = '$afdeling' AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%')

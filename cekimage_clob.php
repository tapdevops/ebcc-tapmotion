<?php
	...
	echo "<b>LAST RUN : ".date("d-m-Y H:i:s")."</b><BR><BR>";
	
	if (in_array(date("H"), array("00", "01", "02", "03"))) {
		echo "DO NOTHING";
	} else {	
		...
		//selisih tanggal skg s/d awal tahun
		$date1 = strtotime("2015-01-01");
		$date2 = strtotime("now");
		$seconds_diff = abs($date1 - $date2); 
		$interval_day = (ceil(($seconds_diff/3600)/24)) - 1;
		
		$sql  = "select ID_BA, NAMA_BA from T_BUSSINESSAREA where ID_BA = '".$par_ba."' group by ID_BA,NAMA_BA ";
		...
		while(oci_fetch($resultPt)){
			...
			for ($i=0;$i<=$interval_day;$i++){
				...
				echo "<br>$font PT : $kodePt - $namaPt Tanggal : $tglTampil Jumlah Image Database : ".count($image) ." - Jumlah Image File =" . $jumahImageCocok ." - Selisih = $selisih </font>";
			}
		echo "<br>";
		}
	}

...
if(isset($_SESSION["subID_CC"]) && isset($_SESSION["subID_BA_Afd"]) && isset($_SESSION["NIK"])){
...
$rbtn_type = $_POST['rbtn_type'];
	...
	if($rbtn_type == 'tanggal' && $date1 == '1970-01-01'){
		$_SESSION[err] = "Date cannot be empty";
		...
		if($NIK == ""){
			...
			if($rbtn_type == 'tanggal'){
				$sql_bcc_restan  = "SELECT TBA.ID_CC,
									 tba.ID_BA,
									 ID_AFD,
									 to_char(sysdate, 'DD-MON-yy') TGL_REPORT,
									 TANGGAL_RENCANA,
									 TB.ID_BLOK ID_BLOK,
									 TB.BLOK_NAME BLOK_NAME,
									 NIK_KERANI_BUAH,
									 EMP_NAME as NAMA_KERANI_BUAH,
									 NO_BCC,
									 F_GET_BJR (TB.ID_BLOK, THRP.TANGGAL_RENCANA, TDRP.ID_BA_AFD_BLOK) BJR,
									 F_GET_HASIL_PANEN_BUNCH (TBA.ID_BA, thp.no_rekap_bcc, thp.no_bcc, 'BUNCH_SEND') AS TBS,
								...
								FROM T_BUSSINESSAREA TBA
									 ...
									 INNER JOIN T_DETAIL_RENCANA_PANEN TDRP
										ON TB.ID_BA_AFD_BLOK = TDRP.ID_BA_AFD_BLOK
									 INNER JOIN T_HEADER_RENCANA_PANEN THRP
									 ...
									 INNER JOIN T_EMPLOYEE TE
										ON TE.NIK = THRP.NIK_KERANI_BUAH									 				 
								WHERE TBA.ID_CC = '$ID_CC'
									 ...
									 AND THP.ID_NAB_TGL = '0'
									 AND THP.STATUS_BCC = 'RESTAN'									 
							ORDER BY THRP.TANGGAL_RENCANA, THP.NO_BCC";
			}else if($rbtn_type == '315'){
				$sql_bcc_restan  = "SELECT TBA.ID_CC,
									 tba.ID_BA,
									 ID_AFD,
									 to_char(sysdate, 'DD-MON-yy') TGL_REPORT,
								...
								FROM T_BUSSINESSAREA TBA
									 ...
									 AND TBA.ID_BA = '$ID_BA'
									 AND TA.ID_AFD = NVL (DECODE ('$valueAfdeling', 'ALL', NULL, '$valueAfdeling'), TA.ID_AFD)
									 AND TO_CHAR (THRP.TANGGAL_RENCANA, 'YYYY-MM-DD') BETWEEN to_char(sysdate-14, 'YYYY-MM-DD')
																  AND  NVL (to_char(sysdate-2, 'YYYY-MM-DD'), to_char(sysdate-2, 'YYYY-MM-DD'))
									 AND THP.ID_NAB_TGL = '0'
									 AND THP.STATUS_BCC = 'RESTAN'
									 ORDER BY THRP.TANGGAL_RENCANA, THP.NO_BCC";
			}else if($rbtn_type == '1630'){
				$sql_bcc_restan  = "SELECT TBA.ID_CC,
									 tba.ID_BA,
									 ID_AFD,
									 to_char(sysdate, 'DD-MON-yy') TGL_REPORT,
								...
								WHERE TBA.ID_CC = '$ID_CC'
									 AND TBA.ID_BA = '$ID_BA'
									 AND TA.ID_AFD = NVL (DECODE ('$valueAfdeling', 'ALL', NULL, '$valueAfdeling'), TA.ID_AFD)
									 AND TO_CHAR (THRP.TANGGAL_RENCANA, 'YYYY-MM-DD') BETWEEN to_char(sysdate-29, 'YYYY-MM-DD')
																  AND  NVL (to_char(sysdate-15, 'YYYY-MM-DD'), to_char(sysdate-15, 'YYYY-MM-DD'))
									 AND THP.ID_NAB_TGL = '0'
									 AND THP.STATUS_BCC = 'RESTAN'									 
							ORDER BY THRP.TANGGAL_RENCANA, THP.NO_BCC";
			}
			...
			header("Location:daftarbccrestan.php?date1=".$date1."&date2=".$date2."&rbtn_type=".$rbtn_type);
		}
	}

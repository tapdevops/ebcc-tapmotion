        ...
        <tr>
          <td>
            <TABLE width="450" class="bordered" bgcolor="#FFFFFF" align="center">
			  ...
			  t_hasilpanen_kualtas H  WHERE  H.ID_BCC(+)='$NO_BCC' AND H.ID_RENCANA = '$ID_RENCANA' AND K.ID_KUALITAS = H.ID_KUALITAS(+) AND GROUP_KUALITAS='HASIL PANEN' AND UOM<>'KG'
        	 ...
        	 WHERE  H.ID_BCC(+)='$NO_BCC' AND H.ID_RENCANA = '$ID_RENCANA' AND K.ID_KUALITAS = H.ID_KUALITAS(+) AND GROUP_KUALITAS='KONDISI BUAH'
			...
			t_hasilpanen_kualtas H  
        	WHERE  H.ID_BCC(+)='$NO_BCC' AND H.ID_RENCANA = '$ID_RENCANA' AND K.ID_KUALITAS = H.ID_KUALITAS(+) AND GROUP_KUALITAS='PENALTY DI TPH'
            ORDER BY ID_KUALITAS ASC";
			...
			t_hasilpanen_kualtas H   WHERE  H.ID_BCC(+)='$NO_BCC' AND H.ID_RENCANA = '$ID_RENCANA' AND K.ID_KUALITAS = H.ID_KUALITAS(+) AND GROUP_KUALITAS='PENALTY MANDOR'
            ORDER BY ID_KUALITAS ASC
			";
			...
			$get_kualitas_penalty = oci_parse($con,"SELECT * FROM T_KUALITAS_PANEN WHERE GROUP_KUALITAS = 'PENALTY MANDOR' ORDER BY ID_KUALITAS ASC");
			oci_execute($get_kualitas_penalty, OCI_DEFAULT);
			while ($p=oci_fetch($get_kualitas_penalty)){
				$daftar_kualitas_penalty[] = oci_result($get_kualitas_penalty, "ID_KUALITAS");
			}
			...
			while(oci_fetch($result_t_hasil2)){
				...
				if(($key = array_search(oci_result($result_t_hasil2, "ID_KUALITAS"), $daftar_kualitas_penalty)) !== false) {
					unset($daftar_kualitas_penalty[$key]);
				}
			...
			foreach($daftar_kualitas_penalty as $row_penalty){
				...
				oci_execute($get_kualitas_penalty, OCI_DEFAULT);
				while ($p=oci_fetch($get_kualitas_penalty)){
					echo "<TR><TD>".oci_result($get_kualitas_penalty, "NAMA_KUALITAS")."</TD><TD align='right'>0&nbsp;</TD><TD>".oci_result($get_kualitas_penalty, "UOM")."</TD></TR>";
				}
			}
//$TOTP=0;
...
?>
  ...
  <tr>
  <td align="center"><input type="button" value="Cetak" onClick="print_save('<?= $ID_RENCANA ?>','<?= $NO_BCC ?>')">&nbsp;<input type="button" class="box2" value="Kembali" onClick="javascript: history.go(-1);"></td>
  </tr>
  <tr>
    <th align="center"><?php

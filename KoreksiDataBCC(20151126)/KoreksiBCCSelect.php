...
$ID_Group_BA = $_SESSION['ID_Group_BA'];
  ...
  from t_header_rencana_panen thrp
     ...
     inner join t_hasilpanen_kualtas thpk
		  ...
		  and tdrp.id_rencana=thpk.id_rencana
...
where     thp.no_bcc = '$NO_BCC'
		...
		$sql_t_BCC_table = "SELECT ID_BCC_KUALITAS,
								 tkp.id_kualitas,
								 tkp.nama_kualitas,
...
{
	//document.getElementById('NIK_Pemanen').value = x;
	//alert('Pastikan koreksi data Anda telah mendapatkan persetujuan dari EM atau KABUN !!!');
...
echo "<td>$no</td>
			...
			<input type=\"hidden\" id=\"ID_Kualitas$xJAN\" value=\"$ID_Kualitas[$xJAN]\">
            ...
            <td align=\"center\">
			<input name=\"NewQty$xJAN\" type=\"text\" id=\"NewQty$xJAN\" value=\"$QTY[$xJAN]\" style=\"width: 50px; height:25px; font-size:15px\" onchange=\"cekvalid($xJAN)\"  /></td>
			";

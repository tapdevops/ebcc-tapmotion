...
$userlogin = $_SESSION["NIK"]; 
		...
		for ($counter = 0; $counter < $roweffec; $counter++)
		{
			...
			{
				$ID_CCfix[$xcount] 				= $ID_CC[$counter];
				$ID_ESTATEfix[$xcount] 			= $ID_ESTATE[$counter];
		...
		//if($roweffec > 0){
			...
			->setCellValue("O1", "COMP_CODE")
			->setCellValue("P1", "ESTATE_CODE");
			...
			for ($x = 2; $x < ($xcount+3) ; $x++) {
				...
				$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue("O$x", "$ID_CCfix[$y]")
							->setCellValue("P$x", "$ID_ESTATEfix[$y]");

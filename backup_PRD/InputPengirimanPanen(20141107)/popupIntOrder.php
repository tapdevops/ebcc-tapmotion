...
<HTML>
<HEAD>
<TITLE>Daftar Internal Order</TITLE>
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			while ($ar=oci_fetch_array($sql)){
				?>
					<tr>
					<td align='center'><u><a href="javascript:pick_iorder('<?= $ar['ID_INTERNAL_ORDER'] ?>');javascript:pick_polisi('<?= $ar['NO_POLISI'] ?>')"><?= $ar['ID_INTERNAL_ORDER'] ?></a></u></td>
						<td align='center'><?= $ar['NO_POLISI'] ?></td>
				</tr>
				<?php
			}
		}
		while ($arr=oci_fetch_array($sql)){
			?>
			<tr>
				<td align='center'><u><a href="javascript:pick_iorder('<?= $arr['ID_INTERNAL_ORDER'] ?>');javascript:pick_polisi('<?= $arr['NO_POLISI'] ?>')"><?= $arr['ID_INTERNAL_ORDER'] ?></a></u></td>
				<td align='center'><?= $arr['NO_POLISI'] ?></td>
			</tr>
			<?PHP
		}
	}

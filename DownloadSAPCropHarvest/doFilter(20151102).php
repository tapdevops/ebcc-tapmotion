...
if(isset($_POST["Afdeling"]) || isset($_POST["date1"]) || isset($_POST["date2"])){
	...
	if($date1 == '1970-01-01'){
		...
		$sql_Download_Crop_Harv = "	
         ...
         THP.NO_TPH,
         CASE
            WHEN TA2.ID_BA <> TA.ID_BA THEN 'CINT_' || TA.ID_BA
            ELSE NULL
         END
            CUST,
         CASE WHEN TA2.ID_BA <> TA.ID_BA THEN TA.ID_BA ELSE NULL END PLANT,
         TB.ID_BLOK,
		 ...
		 NVL (F_GET_HASIL_PANEN_BUNCH (TBA.ID_BA, thp.no_rekap_bcc, thp.no_bcc,  'BUNCH_SEND'), 0) AS DIKIRIM,
         --'' DIKIRIM,
         CASE
   ...
   WHERE     TBA.ID_CC = '$ID_CC' AND
         ...
         AND TA2.ID_AFD = NVL (DECODE ('$valueAfdeling', 'ALL', NULL, '$valueAfdeling'), TA2.ID_AFD)

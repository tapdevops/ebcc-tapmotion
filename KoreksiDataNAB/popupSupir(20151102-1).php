...
function pick(symbol,row) {
    ...
    window.opener.document.inputBCC.NIK_Supir1.value = symbol;
	window.opener.document.editNAB.s_Supir.value = symbol;
	window.close();
	}
}
function pickNAMA(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputBCC.Nama_Supir1.value = symbol;
	window.close();
	}
}
...
IF ($baris=="1"){
	$query = "select COUNT(TJ.ID_BA) as JUMLAH
                 ...
                 where AUTHORITY = '14' and ID_BA_AFD = '$afdeling'";
	...
	$query_emp = "select TJ.ID_BA as BA, 
                 ...
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '14' and ID_BA_AFD = '$afdeling' and INACTIVE_DATE is NULL
						 order by TE.JOB_CODE, EMP_NAME";
	...
	if ($numRow[0]==0) {
		...
		spacing=0 cellpadding=0 style='border:none'>
		<tr>
			<td align='center'>
		...
		if($_POST) {
			...
			$query_emp =   "select TJ.ID_BA as BA, 
							...
							TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
						 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
						 where AUTHORITY = '14' and ID_BA_AFD = '$afdeling' AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%')and INACTIVE_DATE is NULL
...
ELSE{
	...
	$query = "select COUNT(TJ.ID_BA) as JUMLAH
				 from T_JOBAUTHORITY TJ 
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '14' AND (" . $id_ba . ")"; //added by NB 01.07.2014
	$sql1 = oci_parse($con,$query);
			oci_execute($sql1, OCI_DEFAULT);
	$numRow = oci_fetch_row($sql1);
	...
	$query_emp = "select TJ.ID_BA as BA, 
					...
					TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
                 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
                 where AUTHORITY = '14' AND (" . $id_ba . ") and INACTIVE_DATE is NULL
						 order by TJ.ID_BA, SUBSTR(TE.ID_BA_AFD, 5,1), TE.JOB_CODE, EMP_NAME"; //added by NB 01.07.2014
	$sql = oci_parse($con,$query_emp);
			oci_execute($sql, OCI_DEFAULT);
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			$query_emp =   "select TJ.ID_BA as BA, 
							...
							TE.JOB_CODE as JOB_CODE from T_JOBAUTHORITY TJ 
						 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
						 where AUTHORITY = '14' AND (" . $id_ba . ") AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%') and INACTIVE_DATE is NULL
						 order by TJ.ID_BA, SUBSTR(TE.ID_BA_AFD, 5,1), TE.JOB_CODE, EMP_NAME";
			$sql = oci_parse($con,$query_emp);
			oci_execute($sql, OCI_DEFAULT);
			$results=array();

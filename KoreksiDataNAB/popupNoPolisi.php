...
function pick_polisi(symbol,row) {
    ...
    window.opener.document.inputBCC.No_PolisiLabel1.value = symbol;
	window.opener.document.editNAB.sNO_POLISI.value = symbol;	
	window.close();
	}
}
function pick_iorder(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputBCC.Id_Internal_OrderLabel1.value = symbol;
	window.opener.document.editNAB.sID_INTERNAL_ORDER1.value = symbol;

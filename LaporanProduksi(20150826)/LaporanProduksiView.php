		...
		if($LaporanProduksi == TRUE){
			...
			if(isset($_GET["submitvalue"]) == 1){
				$sql_t_hpk  = "SELECT TANGGAL,
									 ...
									 SUM (TBS) TBS,
									 SUM (BRD) BRD
								FROM (  SELECT TN.TGL_NAB TANGGAL,
											   TBA.ID_CC,
											   TBA.ID_BA,
											   TA.ID_AFD,
											   TB.ID_BLOK,
											   ...
											   TDRP.LUASAN_PANEN,
											   TBJ.bjr,
											   SUM (f_get_hasil_panen (thp.no_rekap_bcc, thp.no_bcc, 'TBS'))
												  estimasi_berat,
											   SUM (F_GET_HASIL_PANEN (THP.NO_REKAP_BCC, THP.NO_BCC, 'TBS2'))
												  TBS,
											   SUM (F_GET_HASIL_PANEN (THP.NO_REKAP_BCC, THP.NO_BCC, 'BRD'))
												  BRD
										  FROM T_HEADER_RENCANA_PANEN THRP
											   ...
											   INNER JOIN T_BJR TBJ
												  ON tb.id_ba_afd_blok = tbj.id_ba_afd_blok
										where tba.id_cc = '$subID_CC' and tba.ID_BA = '$sID_BA' 
												...
												and to_char(SPMON,'yyyy-mm-dd') = to_char(trunc(trunc(THRP.TANGGAL_RENCANA,'MM')-1,'MM'),'yyyy-mm-dd')
									  GROUP BY TN.TGL_NAB,
											   TBA.ID_CC,
											   TBA.ID_BA,
											   TA.ID_AFD,
											   ...
											   F_GET_HASIL_PANEN (THP.NO_REKAP_BCC, THP.NO_BCC, 'TBS2'),
...
<table width="1151" height="390" border="0" align="center">
  ...
  <tr>
    <th height="197" scope="row" align="center"><table width="937" border="0" id="setbody2">
      <tr>
        <td colspan="3" align="left" valign="baseline"><span style="font:bold; font-size:20px; font-weight: bold;"><strong>LAPORAN TAKSASI PRODUKSI</strong></span></td>

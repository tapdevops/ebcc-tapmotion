			...
			if($rbtn_filter == "Pemanen"){
				...
				FROM (  SELECT tgl_panen,
						  ...
						  FROM (  SELECT tc.id_cc,
										 ...
										 tdrp.no_rekap_bcc,
										 NVL (
											F_GET_HASIL_PANEN_TBS2 (thp.no_rekap_bcc,
																	thp.no_bcc),
											0)
											AS TBS,
										 NVL (
											F_GET_HASIL_PANEN_BRD (thp.no_rekap_bcc,
																   thp.no_bcc),
											0)
											AS BRD,
										 NVL (
											F_GET_HASIL_PANEN_NUMBER (thp.no_rekap_bcc,
																	  thp.no_bcc,
																	  1),
											0)
											AS BM,
									...
									FROM t_header_rencana_panen thrp
										 ...
										 INNER JOIN t_companycode tc
											ON tba.id_cc = tc.id_cc
										 INNER JOIN SAP_STAGING.ZEST_BLOCKH zb
											ON CONCAT (CONCAT (zb.comp_code, zb.est_code),

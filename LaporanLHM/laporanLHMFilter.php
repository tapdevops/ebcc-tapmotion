		...
		if($username == ""){
			...
			if($LaporanLHM == TRUE){
				...
				if(isset($_SESSION["Afdeling"])){
					...
					$conditionAfd1 = "AND SUBSTR(ID_BA_AFD_BLOK, 5,1) = '$Ses_ID_Afd[0]'";
						...
						<table width="1095" border="0" id="detail">
							...
							<tr>
								...
								<td valign="top" ><form id="form1" name="form1" method="post" action="">
									<?php
										$sql_PMN = "select NIK, EMP_NAME from T_employee where exists (
													select 1
													 from T_HEADER_RENCANA_PANEN THRP, T_DETAIL_RENCANA_PANEN TDRP
													WHERE THRP.ID_RENCANA = TDRP.ID_RENCANA
													AND SUBSTR(ID_BA_AFD_BLOK, 1,4) = '$subID_BA_Afd' $conditionAfd1 $conditionMD
													AND NIK_PEMANEN = NIK)";
								...
								<td colspan='6'>
									...
									<table width='1095' border='0' id='tbl_rekap_blok' style='display:none;'>
										<tr>
											...
											<td valign="top" ><form id="form1" name="form1" method="post" action="">
												<?php
													$sql_PMN = "select ID_BLOK, BLOK_NAME from EBCC.T_BLOK TB where exists (
																select 1
																 from EBCC.T_HEADER_RENCANA_PANEN THRP, EBCC.T_DETAIL_RENCANA_PANEN TDRP
																WHERE THRP.ID_RENCANA = TDRP.ID_RENCANA
																AND SUBSTR(ID_BA_AFD_BLOK, 1,4) = '$subID_BA_Afd' $conditionAfd1 $conditionMD
																AND TDRP.ID_BA_AFD_BLOK = TB.ID_BA_AFD_BLOK
																)";
													//echo $sql_PMN;die();
													$result_PMN = oci_parse($con, $sql_PMN);
													oci_execute($result_PMN, OCI_DEFAULT);

<?php
  ...
  $sql_t_BA  = "select te.nik, te.emp_name from t_employee te inner join 
               t_afdeling ta on te.id_ba_afd = ta.id_ba_afd where ID_BA='$kota' 
			   and id_jobauthority in(select id_jobauthority from t_job_employee where authority=2 group by id_jobauthority) order by te.emp_name ";

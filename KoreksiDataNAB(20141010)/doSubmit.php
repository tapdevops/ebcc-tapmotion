...
if($tglNAB <> '01-01-1970'){
	$tglNAB = date("m-d-Y", strtotime($tglNAB));
		...
		if($No_NAB <> ''){
			$sql_update_NAB = "UPDATE T_NAB set NO_NAB = '" . $No_NAB . "' where ID_NAB_TGL = '" . $ID_NAB_TGL . "'";
			//echo $sql_update_NAB; die();
			$roweffec_value_NAB = num_rows($con,$sql_update_NAB);
		}
		if($tglNAB <> '01-01-1970'){
			$sql_update_tglNAB = "UPDATE T_NAB set TGL_NAB = to_date('" . $tglNAB . "','mm-dd-yyyy hh24:mi:ss') where ID_NAB_TGL = '" . $ID_NAB_TGL . "'";
			//echo $sql_update_tglNAB; die();
			$roweffec_value_tglNAB = num_rows($con,$sql_update_tglNAB);
		}
		...
		if($roweffec_select >0){
			if($roweffec_value_NAB == 0 && $roweffec_value_tglNAB == 0){
			...
			if($roweffec_value > 0 && $result_value_t_log_nab > 0 ){
				...
				}else if($No_NAB <> '' && $tglNAB == '01-01-1970'){
					if($roweffec_value_NAB > 0){
						commit($con);
						$_SESSION["err"] = "Data updated";
					}else{
						rollback($con);

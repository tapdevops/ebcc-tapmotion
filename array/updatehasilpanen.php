...
if(strpos($m['message'],'unique constraint') == true)
{
	...
	for($a = 0 ; $a < $row_t_detail_rencana_panen ; $a++ )
	{
		...
		logToFile($ID_Rencana.'NEXT 1'.$sql_t_drp);
		...
		if($jml_drp == 0)
		{
			...
			logToFile($ID_Rencana.'NEXT 2'.$sql_t_detail_rencana_panen);
			...
			$roweffec_value_log_rencana_panen = num_rows($con,$sql_value_log_rencana_panen);
				...
				logToFile($ID_Rencana.'NEXT 3'.$sql_value_log_rencana_panen);
		...
		if($Luasan_Panen[$a]>0)
		{
			...
			logToFile($ID_Rencana.'NEXT 4'.$sql_t_detail_rencana_panen);
			...
			logToFile($ID_Rencana.'NEXT 5'.$sql_value_log_rencana_panen);
			...
			if($roweffec_t_detail_rencana_panen > 0 && $roweffec_t_detail_rencana_panen != 0){ 
			  // && $roweffec_value_log_rencana_panen > 0 && $roweffec_value_log_rencana_panen != 0){
				...
				logToFile($ID_Rencana.'NEXT 6'.$sql_t_header_rencana_panen);
	...
	$panen_insert_count = 0;
	$panen_update_count = 0;
	$nab_insert_count = 0;
	$nab_update_count = 0;
	...
	for($c = 0 ; $c < $row_t_hasil_panen ; $c++ )
	{
		...
		{
			$ID_NAB_Tgl[$c] 	=  trim($_POST["ID_NAB_Tgl$c"]);
			if($ID_NAB_Tgl[$c] !== '' && $ID_NAB_Tgl[$c] !== 0  && $ID_NAB_Tgl[$c] !== null)
			{
				$No_NAB[$c] 			= trim($_POST["No_NAB$c"]);
				$Tgl_NAB[$c] 			= trim($_POST["Tgl_NAB$c"]);
				$Tipe_Order[$c] 		= trim($_POST["Tipe_Order$c"]);
				$ID_Internal_Order[$c] 	= trim($_POST["ID_Internal_Order$c"]);
				$No_Polisi[$c] 			= trim($_POST["No_Polisi$c"]);
				$NIK_Supir[$c] 			= trim($_POST["NIK_Supir$c"]);
				$NIK_Tukang_Muat1[$c] 	= trim($_POST["NIK_Tukang_Muat1$c"]);
				$NIK_Tukang_Muat2[$c] 	= trim($_POST["NIK_Tukang_Muat2$c"]);
				$NIK_Tukang_Muat3[$c] 	= trim($_POST["NIK_Tukang_Muat3$c"]);
			}
			else{
				$ID_NAB_Tgl[$c] 		= "";
				$No_NAB[$c] 			= "";
		...
		if($No_BCC[$c] == "" || $No_Rekap_BCChp[$c] == ""){
			...
			//Edited by Ardo, 23-03-2016
			//GET LATEST ID_RENCANA
			/*$query_get_latest = "
			SELECT *
			  FROM (SELECT ROWNUM, SYNC_SERVER, NEW_VALUE_ID_RENCANA, NEW_VALUE_NO_REKAP_BCC, OLD_VALUE_NO_REKAP_BCC
					  FROM (    SELECT CONNECT_BY_ROOT NEW_VALUE_ID_RENCANA
										  AS NEW_VALUE_ID_RENCANA, NEW_VALUE_NO_REKAP_BCC, OLD_VALUE_NO_REKAP_BCC, SYNC_SERVER, CREEDIT_FROM
								  FROM T_LOG_HASIL_PANEN
								 WHERE     INSERTUPDATE = 'UPDATE'
									   AND ON_TABLE = 't_hasil_panen'
									   AND ON_NO_BCC = '$No_BCC[$c]'
							CONNECT BY NOCYCLE PRIOR OLD_VALUE_ID_RENCANA =
												  NEW_VALUE_ID_RENCANA
									   AND OLD_VALUE_ID_RENCANA =
											 '$ID_Rencana') where 
											 CREEDIT_FROM = 'Website' 
											 AND NEW_VALUE_ID_RENCANA IS NOT NULL
											 order by SYNC_SERVER desc)
			 WHERE ROWNUM = 1
			";*/ //NBU 22.06.2016
			$query_get_latest = "SELECT MAX (SYNC_SERVER) SYNC_SERVER,
										 NEW_VALUE_ID_RENCANA,
										 NEW_VALUE_NO_REKAP_BCC,
										 OLD_VALUE_NO_REKAP_BCC
								...
								GROUP BY NEW_VALUE_ID_RENCANA, NEW_VALUE_NO_REKAP_BCC, OLD_VALUE_NO_REKAP_BCC
								ORDER BY SYNC_SERVER DESC";
								
			$select_get_latest  = select_data($con,$query_get_latest);
			logToFile($new_rencana.'NEXT LOG0'.$query_get_latest);
			$new_rencana = $select_get_latest["NEW_VALUE_ID_RENCANA"];
			$new_rekap_bcc = $select_get_latest["NEW_VALUE_NO_REKAP_BCC"];
			$old_rekap_bcc = $select_get_latest["OLD_VALUE_NO_REKAP_BCC"];
			
			if($new_rencana=="" and $old_rekap_bcc==""){
				$new_rencana = $ID_Rencana;
				$new_rekap_bcc = $No_Rekap_BCChp[$c];
				$old_rekap_bcc = $No_Rekap_BCChp[$c];
				
				
			} else {
				//Cek dulu 
				$cek_ajah = "SELECT * FROM T_HASIL_PANEN WHERE NO_REKAP_BCC = '" . $No_Rekap_BCChp[$c] . "' 
					AND ID_RENCANA = '" . $ID_Rencana . "' AND NO_BCC <> '".$No_BCC[$c]."'";
				logToFile($new_rencana.'NEXT LOG1'.$cek_ajah);
				$select_cek_ajah  = select_data($con,$cek_ajah);
				$cek_bcc = $select_cek_ajah["NO_BCC"];	
				if($cek_bcc==""){
					//Delete T_DETAIL_RENCANA_PANEN
					$query_del_tdrp_old = "DELETE FROM T_DETAIL_RENCANA_PANEN 
					WHERE NO_REKAP_BCC = '" . $No_Rekap_BCChp[$c] . "' 
					AND ID_RENCANA = '" . $ID_Rencana . "'";
					$r_del_tdrp_old = num_rows($con,$query_del_tdrp_old);
					logToFile($new_rencana.'NEXT LOG2'.$query_del_tdrp_old);
				}
			}
			
			$sql_t_hasil_panen_old = "select Status_BCC from t_hasil_panen 
			WHERE No_BCC  = '$No_BCC[$c]' AND ID_RENCANA = '$new_rencana'";
			$select_t_hasil_panen  = select_data($con,$sql_t_hasil_panen_old);
			$old_Status_BCC = $select_t_hasil_panen["STATUS_BCC"];
			
			
			logToFile($ID_Rencana.'NEXT 7'.$sql_t_hasil_panen_old);
			...
			else
			{
				...
				$sql_t_hp = "select count(*) JUMLAH_DATA from t_hasil_panen where id_rencana = '$new_rencana' and No_BCC = '$No_BCC[$c]'"; 
				$select_t_hp  = select_data($con,$sql_t_hp);
				$jml_hp = $select_t_hp["JUMLAH_DATA"];
				
			
				
				logToFile($ID_Rencana.'NEXT 8'.$sql_t_hp);
				
				if($jml_hp == 0)
				{
					$sql_t_hasil_panen = "INSERT INTO t_hasil_panen 
					(ID_RENCANA, No_Rekap_BCC, No_TPH, No_BCC, Kode_Delivery_Ticket, Latitude, Longitude, Picture_Name, Status_BCC, ID_NAB_Tgl, Status_Tph, Status_Detic, Keterangan_Qrcode)
					VALUES 
					('$new_rencana', '$No_Rekap_BCChp[$c]', '$No_TPH[$c]', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', '$Latitude[$c]', '$Longitude[$c]', '$Picture_Name[$c]', '$Status_BCC[$c]', '$ID_NAB_Tgl[$c]', '$Status_TPH[$c]', '$Status_Detic[$c]', '$Keterangan_Qrcode[$c]')";
					/*$sql_t_hasil_panen = "INSERT INTO t_hasil_panen 
					(ID_RENCANA, No_Rekap_BCC, No_TPH, No_BCC, Kode_Delivery_Ticket, Latitude, Longitude, Picture_Name, Status_BCC, ID_NAB_Tgl) 
					VALUES
					('$new_rencana','$No_Rekap_BCChp[$c]', '$No_TPH[$c]', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', 
					'$Latitude[$c]', '$Longitude[$c]', '$Picture_Name[$c]', '$Status_BCC[$c]', '$ID_NAB_Tgl[$c]')";*/
					$roweffec_t_hasil_panen = num_rows($con,$sql_t_hasil_panen);
					$panen_insert_count = $panen_insert_count + 1;
					
				
					
					logToFile($ID_Rencana.'NEXT 9'.$sql_t_hasil_panen);

					$sql_value_log_hasil_panen = "INSERT INTO t_log_hasil_panen 
					(InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_No_BCC, On_Kode_Delivery_Ticket, New_Value_Status_BCC, Old_Value_Status_BCC, CreEdit_From, Sync_Server, New_Value_Status_TPH, New_Value_Status_Detic) 
					VALUES
					('INSERT', SYSDATE, '$NIK', '$Login_Name', 't_hasil_panen', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', '$Status_BCC[$c]', '$old_Status_BCC', 'Device', SYSDATE, '$Status_TPH[$c]', '$Status_Detic[$c]')" ;
					$roweffec_value_log_hasil_panen = num_rows($con,$sql_value_log_hasil_panen);
					
					logToFile($ID_Rencana.'NEXT 11'.$sql_value_log_hasil_panen);
					
				}
				else
				{
					$sql_t_hasil_panen = "UPDATE t_hasil_panen 
					SET Status_BCC = '$Status_BCC[$c]', ID_NAB_TGL = '$ID_NAB_Tgl[$c]', Status_TPH = '$Status_TPH[$c]', Status_Detic = '$Status_Detic[$c]', Keterangan_Qrcode = '$Keterangan_Qrcode[$c]' 
					WHERE No_BCC  = '$No_BCC[$c]' AND ID_RENCANA = '$new_rencana'";
					$roweffec_t_hasil_panen  = num_rows($con,$sql_t_hasil_panen);
					$panen_update_count = $panen_update_count + 1;
					$jaga = 1;
				
						logToFile($ID_Rencana.'NEXT 10'.$sql_t_hasil_panen);

					$sql_value_log_hasil_panen = "INSERT INTO t_log_hasil_panen 
					(InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_No_BCC, On_Kode_Delivery_Ticket, New_Value_Status_BCC, Old_Value_Status_BCC, CreEdit_From, Sync_Server, New_Value_Status_TPH, New_Value_Status_Detic) 
					VALUES
					('UPDATE', SYSDATE, '$NIK', '$Login_Name', 't_hasil_panen', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', '$Status_BCC[$c]', '$old_Status_BCC', 'Device', SYSDATE, '$Status_TPH[$c]', '$Status_Detic[$c]')" ;
					$roweffec_value_log_hasil_panen = num_rows($con,$sql_value_log_hasil_panen);
				...
				//&& $roweffec_value_log_hasil_panen > 0 && $roweffec_value_log_hasil_panen != 0){
					...
					$hp_array[$c] = 1;
						...
						$sql_check_nab = "SELECT * FROM t_nab where ID_NAB_Tgl = '$ID_NAB_Tgl[$c]'";	
							...
							if ($fetch_check_nab = select_data($con,$sql_check_nab)){								
								...
								logToFile($ID_Rencana.'NEXT 12'.$sql_check_nab);
								...
								$nab_insert_count = $nab_insert_count + 1;
								
							//		logToFile("sql.log", $sql_t_nab);
								
									logToFile($ID_Rencana.'NEXT 13'.$sql_t_nab);
								...
								$sql_value_t_log_nab = "INSERT INTO t_log_nab 
									...
									logToFile($ID_Rencana.'NEXT 14'.$sql_value_t_log_nab);
									commit($con);
					...
					/* if($jaga==1){
						...
						$query_search_thp_avail = "SELECT count(*) JUMLAHHASIL FROM T_HASIL_PANEN 
						WHERE NO_BCC <> '" . $No_BCC[$c] . "' 
						AND ID_RENCANA = '" . $new_rencana . "'
						AND NO_REKAP_BCC = '" . $new_rekap_bcc . "'";
						$select_thp_avail  = select_data($con,$query_search_thp_avail);
						$cek_result_search_thp_avail = $select_thp_avail["JUMLAHHASIL"];
						logToFile($ID_Rencana.'NEXT LOG1'.$query_search_thp_avail);
						if ($cek_result_search_thp_avail==0){
							//delete t_detail_rencana_panen
							$query_del_tdrp_old = "DELETE FROM T_DETAIL_RENCANA_PANEN 
							WHERE NO_REKAP_BCC = '" . $old_rekap_bcc . "' 
							AND ID_RENCANA = '" . $ID_Rencana . "'";
							$r_del_tdrp_old = num_rows($con,$query_del_tdrp_old);
							logToFile($ID_Rencana.'NEXT LOG2'.$query_del_tdrp_old);
						}
					}
					commit($con); */
				}
	...
	*/
	...
	
	{
		...
		if($result_Status_BCC !== "LOST")
		{
			...
			if($jaga!=1){
					...
					logToFile($ID_Rencana.'NEXT 15'.$sql_t_hasilpanen_kualtas);
					...
					if(!$x_exe)
					{
						...
						$logAction ="UPDATE";
						logToFile($ID_Rencana.'NEXT 16'.$sql_t_hasilpanen_kualtas);
					}
					else
					{
					...
					$roweffec_value_log_hasilpanen_kualitas = num_rows($con,$sql_value_log_hasilpanen_kualitas);
						...
						logToFile($ID_Rencana.'NEXT 17'.$sql_value_log_hasilpanen_kualitas);
	...
	$jml_dg = $select_t_dg["JUMLAH_DATA"];
		logToFile($ID_Rencana.'NEXT 18'.$sql_t_dg);
	...
	if($jml_dg == 0)
	{
		...
		{	
			...
			if($NIK_Gandeng[$b] == ""){
				...
				logToFile($ID_Rencana.'NEXT 19'.$sql_dual);
			...
			//	logToFile("sql.log", $sql_t_detail_gandeng);
					logToFile($ID_Rencana.'NEXT 20'.$sql_t_detail_gandeng);
				...
				$sql_value_t_log_detail_gandeng = "INSERT INTO t_log_detail_gandeng (
					...
					//logToFile("sql.log", $sql_value_t_log_detail_gandeng);
						...
						logToFile($ID_Rencana.'NEXT 21'.$sql_value_t_log_detail_gandeng);
	...
	if($x_continue == true)
	{
		...
		if($x_count_lost>0)
		{
			...
			//$response["message"] = $panen_insert_count . " data hasil panen berhasil disimpan, " . $panen_update_count . " data hasil panen berhasil di update, " .$nab_insert_count. " data nab berhasil di input. ".$lost_list;	
			//$response["message"] = $mess . $lost_list;
			$bcc_terkirim = $panen_insert_count + $panen_update_count;
			$bcc_tidak_terkirim = $row_t_hasil_panen - ($panen_insert_count + $panen_update_count);
			$gambar_terkirim = $panen_insert_count + $panen_update_count;
			$gambar_tidak_terkirim = $row_t_hasil_panen - ($panen_insert_count + $panen_update_count);
			$response["message"] = "BCC Terkirim : $bcc_terkirim\nBCC Tidak Terkirim : $bcc_tidak_terkirim\nGambar Terkirim : $gambar_terkirim\nGambar Tidak Terkirim : $gambar_tidak_terkirim\n$lost_list
			";
		...
		else
		{
			...
			//$response["message"] = $panen_insert_count . " data hasil panen berhasil disimpan, " . $panen_update_count . " data hasil panen berhasil di update, " .$nab_insert_count. " data nab berhasil di input.";
			/*$mess = "BCC Terkirim : " . $panen_insert_count + $panen_update_count . "\n";
			$mess .= "BCC Tidak Terkirim : " . $row_t_hasil_panen - ($panen_insert_count + $panen_update_count) . "\n";
			$mess .= "Gambar Terkirim : " . $panen_insert_count + $panen_update_count . "\n";
			$mess .= "Gambar Tidak Terkirim : " . $row_t_hasil_panen - ($panen_insert_count + $panen_update_count) . "\n";*/

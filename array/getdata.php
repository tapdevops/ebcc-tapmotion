...
//logToFile(json_encode($_POST), $_POST . " ini 625");
//logError(json_encode($_POST), $_POST . " ini 625");

//$statusAPK = '1';
//$NIK = "41/4122/0412/401";
//$Login_Name = "4122.1.KB1";
//$subID_BA_Afd = "4122";
...
$x_stage=10;	
					//t_blok
					/* Start Edit Max TPH - Eko Lesmana Sijabat */
					...
					WHERE t2.ID_Group_BA = '$sel_ID_Group_BA[0]' and INACTIVE_DATE is NULL";*/
					$sql_t_blok = "
						SELECT ID_Blok, Blok_Name, ID_BA_Afd, ID_BA_Afd_Blok, Tahun_Tanam, nvl(Max_Tph, 0) as Max_Tph
						FROM (
							SELECT ID_Blok, Blok_Name, ID_BA_Afd, ID_BA_Afd_Blok, Tahun_Tanam,
								CASE
									WHEN MaxTph = 0 
									THEN
										(SELECT to_number(TPH) FROM (SELECT TPH FROM T_BLOK_TPH WHERE werks = SUBSTR(ID_BA_Afd_Blok, 0, 4) AND afd = SUBSTR(ID_BA_Afd_Blok, -4, 1) AND block_code = SUBSTR(ID_BA_Afd_Blok, -3, 3) ORDER BY ID DESC) WHERE ROWNUM = 1)
									ELSE 
										nvl(MaxTph, 0)
								END Max_Tph
							FROM (
								SELECT ID_Blok, Blok_Name, ID_BA_Afd, ID_BA_Afd_Blok, Tahun_Tanam, 
									(SELECT nvl(COUNT(no_tph), 0) FROM tap_dw.tm_tph@devdw_link WHERE werks = SUBSTR(ID_BA_Afd_Blok, 0, 4) AND afd_code = SUBSTR(ID_BA_Afd_Blok, -4, 1) AND block_code = SUBSTR(ID_BA_Afd_Blok, -3, 3)) AS MaxTph
								FROM (
									SELECT 
										DISTINCT t6.ID_Blok, t6.Blok_Name, t6.ID_BA_Afd, t6.ID_BA_Afd_Blok, to_char(t6.Tahun_Tanam, 'YYYY') AS TAHUN_TANAM
									FROM t_alternate_ba_group t2
									INNER JOIN t_bussinessarea t3 	ON t2.ID_BA = t3.ID_BA
									INNER JOIN t_afdeling t5 	ON t3.ID_BA = t5.ID_BA
									INNER JOIN t_blok t6 		ON t5.ID_BA_Afd = t6.ID_BA_Afd
...
$x_stage=17;	
					//t_blok
					$sql_t_blok  = "SELECT DISTINCT t6.ID_Blok, t6.Blok_Name, t6.ID_BA_Afd, t6.ID_BA_Afd_Blok, 
					to_char(t6.Tahun_Tanam, 'YYYY') AS TAHUN_TANAM
					FROM t_bussinessarea t3
					INNER JOIN t_afdeling t5 	ON t3.ID_BA = t5.ID_BA
					INNER JOIN t_blok t6 		ON t5.ID_BA_Afd = t6.ID_BA_Afd
					WHERE t3.ID_BA = '$subID_BA_Afd'";
...
$x_stage=21;	
				//t_kualitas_panen
				$sql_t_kualitas_panen  = "SELECT DISTINCT ID_Kualitas, Nama_Kualitas, UOM, Group_Kualitas, Active_Status, PARAM
				...
				while(oci_fetch($result_t_kualitas_panen)){			
					...
					$KP_PARAM[]	= oci_result($result_t_kualitas_panen, strtoupper("PARAM"));
				}
				$roweffec_t_kualitas_panen = oci_num_rows($result_t_kualitas_panen);
				//echo $sql_t_kualitas_panen.$roweffec_t_kualitas_panen."<br>";
$x_stage=22;	
				//t_bjr
				$sql_t_bjr  = "select a.ID_BA_AFD_blok as id_ba_afd_blok, F_GET_BJR (a.id_blok,trunc(sysdate), a.ID_BA_AFD_BLOK) as bjr 
...
$x_stage=221;	
				...
				$sql_t_ip  = "select Ipimage from t_user  where login_name='$Login_Name'";
				//print_r($sql_t_ip);
				$result_t_ip = oci_parse($con, $sql_t_ip);
				oci_execute($result_t_ip, OCI_DEFAULT);
				while(oci_fetch($result_t_ip)){	
				//	$IPDATA[]	= oci_result($result_t_ip, strtoupper("Ipdata"));				
					$IPIMAGE[]	= oci_result($result_t_ip, strtoupper("Ipimage"));	
					
				}
				$roweffec_t_ip = oci_num_rows($result_t_ip);
...
$x_stage=24;					
					...
					//t_parameter_bunch //added by NBU 30.11.2015
					$sql_t_p_bunch  = "SELECT BA_CODE, ID_KUALITAS, KETERANGAN,
										INSERT_USER, TO_CHAR(INSERT_TIME, 'DD-MM-YYYY') AS INSERT_TIME,
										UPDATE_USER, TO_CHAR(UPDATE_TIME, 'DD-MM-YYYY') AS UPDATE_TIME,
										DELETE_USER, TO_CHAR(DELETE_TIME, 'DD-MM-YYYY') AS DELETE_TIME
										FROM T_PARAMETER_BUNCH WHERE BA_CODE = '$subID_BA_Afd'";
					$result_t_p_bunch = oci_parse($con, $sql_t_p_bunch);
					oci_execute($result_t_p_bunch, OCI_DEFAULT);
					while(oci_fetch($result_t_p_bunch)){
						$bunch_ba_code[]		= oci_result($result_t_p_bunch, strtoupper("BA_CODE"));
						$bunch_id_kualitas[]	= oci_result($result_t_p_bunch, strtoupper("ID_KUALITAS"));
						$bunch_keterangan[]		= oci_result($result_t_p_bunch, strtoupper("KETERANGAN"));
						$bunch_insert_user[]	= oci_result($result_t_p_bunch, strtoupper("INSERT_USER"));
						$bunch_insert_time[]	= oci_result($result_t_p_bunch, strtoupper("INSERT_TIME"));
						$bunch_update_user[]	= oci_result($result_t_p_bunch, strtoupper("UPDATE_USER"));
						$bunch_update_time[]	= oci_result($result_t_p_bunch, strtoupper("UPDATE_TIME"));
						$bunch_delete_user[]	= oci_result($result_t_p_bunch, strtoupper("DELETE_USER"));
						$bunch_delete_time[]	= oci_result($result_t_p_bunch, strtoupper("DELETE_TIME"));
					}
					$roweffec_t_p_bunch = oci_num_rows($result_t_p_bunch);
$x_stage=25;
				...
				$roweffec_t_p_bunch > 0 //added by NBU 30.11.2015
				...
				$roweffec_t_kualitas_panen > 0 */
					...
					$response["t_ip"]	= array();
					$response["t_parameter_bunch"]	= array();
					...
					for($y=0; $y<$roweffec_t_kualitas_panen; $y++){
						...
						$t_kualitas_panen["PARAM"]			= $KP_PARAM[$y];
						array_push($response["t_kualitas_panen"], $t_kualitas_panen);
					}
					
					for($y=0; $y<$roweffec_t_ip; $y++){
						$t_ip = array();
				//		$t_ip["IPDATA"]	= $IPDATA[$y];
						$t_ip["IPIMAGE"]	= $IPIMAGE[$y];
						array_push($response["t_ip"], $t_ip);
					...
					//t_p_bunch
					for($y=0; $y<$roweffec_t_p_bunch; $y++){
						$t_p_bunch[] = array();
						$t_p_bunch["BA_CODE"]			= $bunch_ba_code[$y];
						$t_p_bunch["ID_KUALITAS"]		= $bunch_id_kualitas[$y];
						$t_p_bunch["KETERANGAN"]		= $bunch_keterangan[$y];
						$t_p_bunch["INSERT_USER"]		= $bunch_insert_user[$y];	
						$t_p_bunch["INSERT_TIME"]		= $bunch_insert_time[$y];		
						$t_p_bunch["UPDATE_USER"]		= $bunch_update_user[$y];
						$t_p_bunch["UPDATE_TIME"]		= $bunch_update_time[$y];
						$t_p_bunch["DELETE_USER"]		= $bunch_delete_user[$y];	
						$t_p_bunch["DELETE_TIME"]		= $bunch_delete_time[$y];								
						array_push($response["t_parameter_bunch"], $t_p_bunch);
					}	
					...
					*/
					...
					//logToFile(json_encode($response), json_encode($response). " ini 200");
					//logError(json_encode($response), json_encode($response). " ini 200");
					...
					//rollback($con);
					//logToFile(json_encode($response), json_encode($response). " ini 609");
					//logError(json_encode($response), json_encode($response). " ini 609");
					echo json_encode($response);
				}	
				...
				//logToFile(json_encode($response), json_encode($response). " ini 617");
				//logError(json_encode($response), json_encode($response). " ini 617");
				echo json_encode($response);
			}
			...
			//logToFile(json_encode($response), json_encode($response). " ini 625");
			//logError(json_encode($response), json_encode($response). " ini 625");
			echo json_encode($response);
		}
...
//logToFile(json_encode($response), json_encode($response). " ini 642");
//logError(json_encode($response), json_encode($response). " ini 642");
echo json_encode($response);
} 
?>
...
$x_stage=10;	
					...
					/*$sql_t_blok  = "
						SELECT ID_Blok, Blok_Name, ID_BA_Afd, ID_BA_Afd_Blok, Tahun_Tanam, 
							(SELECT COUNT(no_tph) FROM tap_dw.tm_tph@devdw_link WHERE werks = SUBSTR(ID_BA_Afd_Blok, 0, 4) AND afd_code = SUBSTR(ID_BA_Afd_Blok, -4, 1) AND block_code = SUBSTR(ID_BA_Afd_Blok, -3, 3)) AS Max_Tph
						FROM (
							SELECT 
								DISTINCT t6.ID_Blok, t6.Blok_Name, t6.ID_BA_Afd, t6.ID_BA_Afd_Blok, 
								to_char(t6.Tahun_Tanam, 'YYYY') AS TAHUN_TANAM, '30' AS Max_TPH
							FROM t_alternate_ba_group t2
							INNER JOIN t_bussinessarea t3 	ON t2.ID_BA = t3.ID_BA
							INNER JOIN t_afdeling t5 	ON t3.ID_BA = t5.ID_BA
						...
						FROM (
							...
							FROM (
								...
								FROM (
									...
									INNER JOIN t_afdeling t5 	ON t3.ID_BA = t5.ID_BA
									INNER JOIN t_blok t6 		ON t5.ID_BA_Afd = t6.ID_BA_Afd
									WHERE t2.ID_Group_BA = '001' and INACTIVE_DATE is NULL
								)
							)
					)";
...
$x_stage=25;
				...
				//		$t_ip["IPDATA"]	= $IPDATA[$y];
					...
					*/
					...
					echo str_replace('\/','/',json_encode($response));//die();
				}	
				else{
					$response["success"] = 0;
					...
					//rollback($con);
					echo json_encode($response);
					//logToFile("my.log", $response. " ini 609");
				}	
				...
				echo json_encode($response);
				//logToFile("my.log", $response. " ini 617");
			}
			...
			echo json_encode($response);
			//logToFile("my.log", $response. " ini 625");
		}
...
echo json_encode($response);
//logToFile("my.log", $response. " ini 642");
} 
?>
...
logToFile(json_encode($_POST));
	...
	logToFile($ID_Rencana.'s 1 ='.$Tanggal_Rencana);
	logToFile($ID_Rencana.'s 2='.$NIK_Mandor);
	logToFile($ID_Rencana.'s 3='.$NIK_Pemanen);
	logToFile($ID_Rencana.'s 4='.$Status_Gandeng);
	logToFile($ID_Rencana.'s 5='.$row_t_detail_rencana_panen);
	logToFile($ID_Rencana.'s 6='.$row_t_hasil_panen);
	logToFile($ID_Rencana.'s 7='.$row_t_hasilpanen_kualitas);
	logToFile($ID_Rencana.'s 8='.$row_t_detail_gandeng);
	logToFile($ID_Rencana.'s 9='.$ID_Rencana);
	*/
	...
	if($NIK == "" && $Login_Name == "") {
		...
		if($_POST["row_t_detail_gandeng"] == "") { //added by NB 15.11.2014
			$_POST["row_t_detail_gandeng"] = '0';
			$_POST["Status_Gandeng"] = 'NO';
		} //end added sementara

		if(isset($_POST["Tanggal_Rencana"]) && isset($_POST["NIK_Mandor"]) && isset($_POST["NIK_Pemanen"]) && isset($_POST["Status_Gandeng"]) && isset($_POST["row_t_detail_rencana_panen"]) && isset($_POST["row_t_hasil_panen"]) && isset($_POST["row_t_hasilpanen_kualitas"]) &&
			isset($_POST["row_t_detail_gandeng"]) && isset($_POST["ID_Rencana"])) {
			$Tanggal_Rencana 	= $_POST["Tanggal_Rencana"]; 
			$NIK_Mandor 		= $_POST["NIK_Mandor"];
			$NIK_Pemanen 		= $_POST["NIK_Pemanen"];

			$Status_Gandeng = ($_POST['row_t_detail_gandeng'] == '0') ? 'NO' : $_POST['Status_Gandeng'];
			...
			if($Tanggal_Rencana == "" && $NIK_Mandor == "" && $NIK_Pemanen == "" && $Status_Gandeng == "" && $row_t_detail_rencana_panen == "" && $row_t_hasil_panen == "" && $row_t_hasilpanen_kualitas == "" && $row_t_detail_gandeng == "" && $ID_Rencana == "") {
				$response["success"] = 0;
				$response["message"] = "data yang dikirim kosong";
				echo json_encode($response);
				logError(json_encode($response));
			} else if($Tanggal_Rencana == "" || $NIK_Mandor == "" || $NIK_Pemanen == "" || $Status_Gandeng == "" || $row_t_detail_rencana_panen == "" || $row_t_hasil_panen == "" || $row_t_hasilpanen_kualitas == "" || $row_t_detail_gandeng == "" || $ID_Rencana == "") {
				$response["success"] = 0;
				...
				$sql_t_header_rencana_panen = "
					INSERT INTO t_header_rencana_panen(ID_Rencana, Tanggal_Rencana, NIK_Mandor, NIK_KERANI_BUAH, NIK_Pemanen, Status_Gandeng) 
					VALUES ('$ID_Rencana', to_date('$Tanggal_Rencana','YYYY-MM-DD'), '$NIK_Mandor', '$NIK', '$NIK_Pemanen', '$Status_Gandeng')
				";

				// STEP 1
				logToFile($ID_Rencana.'STEP 1'.$sql_t_header_rencana_panen);
				//for update t_hasil_panen
				$stmt = oci_parse($con,$sql_t_header_rencana_panen);
				$x_exe = oci_execute($stmt, OCI_DEFAULT);
				if(!$x_exe) {
					$m = oci_error($stmt);
					logToFile($ID_Rencana.'STEP TERPISAH'.$stmt);
				...
				if ($roweffec_t_header_rencana_panen > 0 && $roweffec_t_header_rencana_panen != 0) {
					...
					logToFile($ID_Rencana.'STEP 2'.$sql_t_dg);
					...
					if($jml_dg == 0) {
						for($b = 0 ; $b < $row_t_detail_gandeng ; $b++) {
							...
							if($NIK_Gandeng[$b] == "") {
								...
								logToFile($ID_Rencana.'STEP 3'.$sql_dual);

								oci_execute($result_dual, OCI_DEFAULT);
								oci_fetch($result_dual);
								$ID_Gandeng = oci_result($result_dual, "ID_GANDENG");

								$sql_t_detail_gandeng = "
									INSERT INTO t_detail_gandeng (ID_Gandeng, ID_Rencana, NIK_Gandeng) 
									VALUES ('$ID_Gandeng', '$ID_Rencana', '$NIK_Gandeng[$b]')
								";

								$roweffec_t_detail_gandeng = num_rows($con,$sql_t_detail_gandeng);

								logToFile($ID_Rencana.'STEP 4'.$sql_t_detail_gandeng);

								if($roweffec_t_detail_gandeng > 0 && $roweffec_t_detail_gandeng != 0) {
									$x_stage = 7;
									$sql_value_t_log_detail_gandeng = "
										INSERT INTO t_log_detail_gandeng (InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_Gandeng, CreEdit_From, Sync_Server) 
										VALUES ('INSERT', SYSDATE , '$NIK', '$Login_Name', 't_detail_gandeng', '$ID_Gandeng', 'Device', SYSDATE)
									" ;
									$result_value_t_log_detail_gandeng = insert_data($con,$sql_value_t_log_detail_gandeng);

									logToFile($ID_Rencana.'STEP 5'.$sql_value_t_log_detail_gandeng);
									commit($con);
					...
					for($a = 0 ; $a < $row_t_detail_rencana_panen ; $a++ ) {
						...
						//mysql_query("BEGIN");
						$sql_t_detail_rencana_panen = "
							INSERT INTO t_detail_rencana_panen (ID_BA_Afd_Blok, No_Rekap_BCC, ID_Rencana, Luasan_Panen)
							VALUES ('$ID_BA_Afd_Blok[$a]', '$No_Rekap_BCCdr[$a]', '$ID_Rencana', '$Luasan_Panen[$a]' )
						";
						$roweffec_t_detail_rencana_panen = num_rows($con,$sql_t_detail_rencana_panen);

						logToFile($ID_Rencana.'STEP 6'.$sql_t_detail_rencana_panen);

						$sql_value_log_rencana_panen = "
							INSERT INTO t_log_rencana_panen (InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_Rencana, On_No_Rekap_BCC, New_Value_ID_BA_Afd_Blok, CreEdit_From, Sync_Server)
							VALUES ('INSERT', SYSDATE, '$NIK', '$Login_Name', 't_detail_rencana_panen', '$ID_Rencana', '$No_Rekap_BCCdr[$a]', '$ID_BA_Afd_Blok[$a]', 'Device', SYSDATE)
						";
						$roweffec_value_log_rencana_panen = num_rows($con,$sql_value_log_rencana_panen);

						logToFile($ID_Rencana.'STEP 7'.$sql_value_log_rencana_panen);
					...
					if($row_t_detail_rencana_panen == $affected_t_detail_rencana_panen) {
						...
						for($c = 0 ; $c < $row_t_hasil_panen ; $c++ ) {
							...
							$Status_TPH[$c]		=  (isset($_POST["Status_TPH$c"])) ? $_POST["Status_TPH$c"] : '';
							$Status_Detic[$c] 	=  (isset($_POST["Status_Detic$c"])) ? $_POST["Status_Detic$c"] : '';
							$Keterangan_Qrcode[$c] = (isset($_POST["Keterangan_Qrcode$c"])) ? $_POST["Keterangan_Qrcode$c"] : '';
							$image_name[$c] =  $_POST["IMG_NAME$c"];
							$total_scan[$c] =  $_POST["TOTAL_SCAN$c"];
							...
							if($No_BCC[$c] == "" || $No_Rekap_BCChp[$c] == "") {
								...
								//mysql_query("BEGIN");
								/* Start - Update Eko Lesmana Sijabat - 19 Feb 2018 */
								$sql_t_hasil_panen = "
									INSERT INTO t_hasil_panen (ID_RENCANA, No_Rekap_BCC, No_TPH, No_BCC, Kode_Delivery_Ticket, Latitude, Longitude, Picture_Name, Status_BCC, ID_NAB_Tgl, Status_Tph, Status_Detic, Keterangan_Qrcode) 
									VALUES ('$ID_Rencana','$No_Rekap_BCChp[$c]', '$No_TPH[$c]', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', 
										'$Latitude[$c]', '$Longitude[$c]', '$Picture_Name[$c]', '$Status_BCC[$c]', '$ID_NAB_Tgl[$c]', '$Status_TPH[$c]', '$Status_Detic[$c]', '$Keterangan_Qrcode[$c]')
								";
								$roweffec_t_hasil_panen = num_rows($con,$sql_t_hasil_panen);

								logToFile($ID_Rencana.'STEP 8'.$sql_t_hasil_panen);
								/* End - Update Eko Lesmana Sijabat - 19 Feb 2018 */
								
								//aries add auto ebcc 04/07/2018
								$sql_t_panen_auto = "
									INSERT INTO t_panen_auto (ID_RENCANA, NO_BCC, PICTURE_NAME, RESULT ) 
									VALUES ('$ID_Rencana', '$No_BCC[$c]', '$image_name[$c]', $total_scan[$c])
								";                                        
								$roweffec_t_panen_auto = num_rows($con,$sql_t_panen_auto);
                                
								logToFile($ID_Rencana.'STEP auto'.$sql_t_panen_auto);
								//==============================
								
								$sql_value_log_hasil_panen = "
									INSERT INTO t_log_hasil_panen (InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_No_BCC, On_Kode_Delivery_Ticket, New_Value_Status_BCC, CreEdit_From, Sync_Server, New_Value_Status_TPH, New_Value_Status_Detic) 
									VALUES ('INSERT', SYSDATE, '$NIK', '$Login_Name', 't_hasil_panen', '$No_BCC[$c]', '$Kode_Delivery_Ticket[$c]', '$Status_BCC[$c]', 'Device', SYSDATE, '$Status_TPH[$c]', '$Status_Detic[$c]')
								";
								
								$roweffec_value_log_hasil_panen = num_rows($con,$sql_value_log_hasil_panen);

								logToFile($ID_Rencana.'STEP 9'.$sql_value_log_hasil_panen);
								...
								if($roweffec_t_hasil_panen > 0 && $roweffec_t_hasil_panen != 0) {
									...
									*/
									...
									if($ID_NAB_Tgl[$c] == "" || $No_NAB[$c] == "" || $Tgl_NAB[$c] == "" || $No_Polisi[$c] == "" || $NIK_Supir[$c] == "") {
										...
										$x_stage = 21;
										$sql_check_nab = "
											SELECT * FROM t_nab where ID_NAB_Tgl = '$ID_NAB_Tgl[$c]' AND No_NAB  = '$No_NAB[$c]' AND Tgl_NAB  = to_date('$Tgl_NAB[$c]', 'YYYY-MM-DD') AND No_Polisi  = '$No_Polisi[$c]' AND NIK_Supir  = '$NIK_Supir[$c]' 
										";

										logToFile($ID_Rencana.'STEP 10'.$sql_check_nab);
										...
										if ($fetch_check_nab = select_data($con,$sql_check_nab)) {
											...
											//mysql_query("BEGIN");
											$sql_t_nab = "
												INSERT INTO t_nab (ID_NAB_Tgl, No_NAB, Tgl_NAB, Tipe_Order, ID_Internal_Order, No_Polisi, NIK_Supir, NIK_Tukang_Muat1, NIK_Tukang_Muat2, NIK_Tukang_Muat3, STATUS_DOWNLOAD)
												VALUES ('$ID_NAB_Tgl[$c]', '$No_NAB[$c]', to_date('$Tgl_NAB[$c]', 'YYYY-MM-DD'),'$Tipe_Order[$c]', '$ID_Internal_Order[$c]', '$No_Polisi[$c]', '$NIK_Supir[$c]', '$NIK_Tukang_Muat1[$c]', '$NIK_Tukang_Muat2[$c]', '$NIK_Tukang_Muat3[$c]', 'N')
											";
											$roweffec_t_nab = num_rows($con,$sql_t_nab);

											logToFile($ID_Rencana.'STEP 11'.$sql_t_nab);

											$sql_value_t_log_nab = "
												INSERT INTO t_log_nab (
													InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_NAB_Tgl, 
													CreEdit_From, Sync_Server, New_Supir, New_Tukang_Muat_1, New_Tukang_Muat_2, 
													New_Tukang_Muat_3, New_Status_Download
												) VALUES (
													'INSERT', SYSDATE, '$NIK', '$Login_Name', 't_nab', '$ID_NAB_Tgl[$c]', 'Device', SYSDATE, 
													'$NIK_Supir[$c]', '$NIK_Tukang_Muat1[$c]', '$NIK_Tukang_Muat2[$c]', '$NIK_Tukang_Muat3[$c]', 'N'
												)
											";

											logToFile($ID_Rencana.'STEP 12'.$sql_value_t_log_nab);

											$result_value_t_log_nab = num_rows($con,$sql_value_t_log_nab);

											if($roweffec_t_nab > 0) {
												//&& $result_value_t_log_nab > 0){
												$x_stage = 24;
												commit($con);
												$affected_t_nab++;
						...
						logToFile($ID_Rencana.'baim 14'.$row_t_hasil_panen);
						logToFile($ID_Rencana.'baim2 14'.$affected_t_hasil_panen);
						...
						if($row_t_hasil_panen == $affected_t_hasil_panen) {
							...
							for($d = 0 ; $d < $row_t_hasilpanen_kualitas ; $d++ ) {
								...
								//mysql_query("BEGIN");
								$sql_t_hasilpanen_kualtas = "
									INSERT INTO t_hasilpanen_kualtas (ID_BCC_Kualitas, ID_BCC, ID_Kualitas, Qty, ID_RENCANA) 
									VALUES ('$ID_BCC_Kualitas[$d]', '$ID_BCC[$d]', '$ID_Kualitas[$d]', '$Qty[$d]', '$ID_Rencana')
								";
								$roweffec_t_hasilpanen_kualtas = num_rows($con,$sql_t_hasilpanen_kualtas);

								logToFile($ID_Rencana.'STEP 13'.$sql_t_hasilpanen_kualtas);

								$sql_value_log_hasilpanen_kualitas = "
									INSERT INTO t_log_hasilpanen_kualitas (InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_BCC_Kualitas, New_Value_Qty, CreEdit_From, Sync_Server)
									VALUES ('INSERT', SYSDATE, '$NIK', '$Login_Name', 't_hasilpanen_kualtas', '$ID_BCC_Kualitas[$d]', $Qty[$d], 'Device', SYSDATE)
								";
								$roweffec_value_log_hasilpanen_kualitas = num_rows($con,$sql_value_log_hasilpanen_kualitas);

								logToFile($ID_Rencana.'STEP 14'.$sql_value_log_hasilpanen_kualitas);
							...
							if($affected_t_hasil_panen == 0) {
								$sql_del_t_header_rencana_panen = "DELETE FROM t_header_rencana_panen WHERE ID_Rencana = '$ID_Rencana'";
								$result_del_t_header_rencana_panen = delete_data($con,$sql_del_t_header_rencana_panen);	

								logToFile($ID_Rencana.'STEP 15'.$sql_del_t_header_rencana_panen);

								for($b = 0 ; $b < $row_t_detail_gandeng ; $b++ ) {
									$sql_del_t_detail_gandeng = "DELETE FROM t_detail_gandeng WHERE  ID_Gandeng = '$ID_Gandeng'";
									$result_del_t_detail_gandeng = delete_data($con,$sql_del_t_detail_gandeng);

									logToFile($ID_Rencana.'STEP 13'.$sql_del_t_detail_gandeng);

									$sql_del_value_t_log_detail_gandeng = "
										DELETE FROM t_log_detail_gandeng
									...
									logToFile($ID_Rencana.'STEP 16'.$sql_del_value_t_log_detail_gandeng);
								}

								for($a = 0 ; $a < $row_t_detail_rencana_panen ; $a++ ) {
									$sql_del_t_detail_rencana_panen = "
										DELETE FROM t_detail_rencana_panen WHERE ID_BA_Afd_Blok = '$ID_BA_Afd_Blok[$a]' AND No_Rekap_BCC = '$No_Rekap_BCCdr[$a]' AND ID_Rencana = '$ID_Rencana' AND Luasan_Panen = '$Luasan_Panen[$a]'
									";
									$result_del_t_detail_rencana_panen = delete_data($con,$sql_del_t_detail_rencana_panen);

									logToFile($ID_Rencana.'STEP 17'.$sql_del_t_detail_rencana_panen);

									$sql_del_value_log_rencana_panen = "
										DELETE FROM t_log_rencana_panen 
											WHERE NIK_CreEditor = '$NIK' 
											AND Login_Name_CreEditor = '$Login_Name' 
									...
									logToFile($ID_Rencana.'STEP 18'.$sql_del_value_log_rencana_panen);
								}
			...
			//# YES # 1 #  #  #  # 20140321.356534058058973.21/2121/0608/248,
			echo json_encode($response);
			logError(json_encode($response));
		}
	}// close else	

...
if(isset($_POST['UpdateAAP']) || isset($_POST['AddAAP']) || isset($_POST['NewAAPSubmit']) || isset($_POST['DelStat'])){
	...
	if($_POST['UpdateAAP'] == "" && $_POST['AddAAP'] == "" && $_POST['NewAAPSubmit'] == "" && $_POST['DelStat'] == ""){
		...
		//echo "1. ".$_POST['UpdateAAP']." - 2. ".$_POST['AddAAP']." - 3.".$_POST['NewAAPSubmit']." - 4. ".$_POST['DelStat']; exit;
		...
		if($_POST['UpdateAAP'] == TRUE){
			...
			$ID_Group_BA 			= $_SESSION['ID_Group_BA'];
				...
				if($roweffec_DETAILGANDENG == "" || $LUASAN_PANEN	== ""){
					...
					$sql_value[$x] = "UPDATE t_detail_rencana_panen SET LUASAN_PANEN = $LUASAN_PANEN WHERE ID_RENCANA = '$ID_RENCANA' AND NO_REKAP_BCC = '$NO_REKAP_BCC'";
					$roweffec_value[$x] = num_rows($con,$sql_value[$x]);
					echo $roweffec_DETAILGANDENG; exit;
		...
		// OPEN TASYA ADD and SUBMIT
			...
			if(isset($_POST['ID_Detail_Gandeng_BARU']) && isset($_POST['ID_Rencana_IGBARU']) && isset($_POST['NIK_Gandeng_BARU']) && isset($_POST['ID_BA_Gandeng']) && isset($_SESSION['LoginName']) && isset($_SESSION['NIK'])){
				...
				if($ID_Detail_Gandeng_BARU == "" || $ID_Rencana_IGBARU == "" || $NIK_Gandeng_BARU  == ""){
					...
					oci_fetch($rs_select_NIKExist);
					$roweffec_select_NIKExist	= oci_num_rows($rs_select_NIKExist);
					//echo "<br> roweffec_select_NIKExist". $roweffec_select_NIKExist. $sql_select_NIKExist; exit;
					
					if($roweffec_select_NIKExist > 0){
						//Edited by Ardo, 03-10-2016 : CR Perubahan Koreksi AAP
						$get_tanggal_pemanen = "select to_char(tanggal_rencana,'MM/DD/YYYY') tanggal_rencana, nik_pemanen from t_header_rencana_panen where id_rencana='$ID_Rencana_IGBARU'";
						...
						$id_gandeng_baru = $ID_Detail_Gandeng_BARU;
						$jml_cek_gandeng = 0;
						$jml_true_gandeng = 0;
						while(oci_fetch($r_get_other_rencana)){
							$jml_true_gandeng++;
							$query_tdg_cek = "SELECT * FROM T_DETAIL_GANDENG 
							WHERE ID_RENCANA = '" . oci_result($r_get_other_rencana, "ID_RENCANA") . "'";
							$result_tdg_cek = select_data($con, $query_tdg_cek);
							if ($result_tdg_cek['ID_RENCANA'] == ""){
								//echo oci_result($r_get_other_rencana, "ID_RENCANA")." tidak ada<br>";
								$sql_InsertNewGandeng = "INSERT INTO t_detail_gandeng
								(ID_GANDENG, ID_RENCANA, NIK_GANDENG) 
								VALUES
								('$id_gandeng_baru', '".oci_result($r_get_other_rencana, "ID_RENCANA")."', '$NIK_Gandeng_BARU')";
								...
								if($roweffec_InsertNewGandeng > 0){
									...
									VALUES
									('INSERT', SYSDATE , '$NIK', '$Login_Name', 't_detail_gandeng', '$id_gandeng_baru', 'Website', SYSDATE)" ;
								...
								//echo oci_result($r_get_other_rencana, "ID_RENCANA")." ada<br>";
								$sql_InsertNewGandeng = "update t_detail_gandeng
								set NIK_GANDENG = '$NIK_Gandeng_BARU'
								where ID_RENCANA = '".oci_result($r_get_other_rencana, "ID_RENCANA")."'";
								$roweffec_InsertNewGandeng = num_rows($con,$sql_InsertNewGandeng);
								...
								if($roweffec_InsertNewGandeng > 0){
									...
									InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_Gandeng, CreEdit_From, Sync_Server) 
									VALUES
									('UPDATE', SYSDATE , '$NIK', '$Login_Name', 't_detail_gandeng', '$id_gandeng_baru', 'Website', SYSDATE)" ;
									$result_value_t_log_detail_gandeng = insert_data($con,$sql_value_t_log_detail_gandeng);
									$jml_cek_gandeng++;
									/* commit($con);
									$_SESSION['err'] = "Add New NIK Gandeng Success";
							...
							if($jml_cek_gandeng==$jml_true_gandeng){
								commit($con);
								$_SESSION['err'] = "Add New NIK Gandeng Success";
								header("location:KoreksiAAPSelect.php");
							} else {
		...
		// OPEN TASYA DELETE
			...
			$Del_NIK_GANDENG = $_POST['NewNIK_GANDENG'.$DelLine];
			$Login_Name 	= $_SESSION['LoginName'];
			$NIK 			= $_SESSION['NIK'];
			...
			$get_tanggal_pemanen = "select to_char(tanggal_rencana,'MM/DD/YYYY') tanggal_rencana, nik_pemanen from t_header_rencana_panen where id_rencana='$Del_IDRencana'";
			$r_get_tanggal_pemanen	= select_data($con,$get_tanggal_pemanen);
			...
			while(oci_fetch($r_get_other_rencana)){
				$jml_true_gandeng++;
				$get_id_gandeng = "select id_gandeng FROM t_detail_gandeng WHERE NIK_GANDENG='$Del_NIK_GANDENG' and ID_RENCANA = '".oci_result($r_get_other_rencana, "ID_RENCANA")."'";
				$r_id_gandeng = select_data($con, $get_id_gandeng);
				
				$sql_DeleteIDGandeng = "DELETE FROM t_detail_gandeng WHERE NIK_GANDENG='$Del_NIK_GANDENG' and ID_RENCANA = '".oci_result($r_get_other_rencana, "ID_RENCANA")."'";
				$roweffec_DeleteIDGandeng = num_rows($con,$sql_DeleteIDGandeng);
				//echo oci_result($r_get_other_rencana, "ID_RENCANA")." ".$roweffec_DeleteIDGandeng."<br>";
				...
				if($roweffec_DeleteIDGandeng > 0){
					...
					InsertUpdate, Tgl_CreEdit, NIK_CreEditor, Login_Name_CreEditor, On_Table, On_ID_Gandeng, CreEdit_From, Sync_Server) 
					VALUES
					('DELETE', SYSDATE , '$NIK', '$Login_Name', 't_detail_gandeng', '".$r_id_gandeng['ID_GANDENG']."', 'Website', SYSDATE)" ;

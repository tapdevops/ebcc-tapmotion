		...
		FROM (
			...
			FROM (
				SELECT 
		    		...
		    		THP.STATUS_TPH,
		    		NVL(THP.STATUS_LOKASI, 0) AS STATUS_LOKASI,
		    		(CASE
                        WHEN
                        ...
                        THEN
                            (CASE
                                WHEN 
                                    tmt.latitude IS NULL OR tmt.longitude IS NULL THEN 30.1
                            ELSE
                                ROUND(SDO_GEOM.SDO_DISTANCE(SDO_GEOM.SDO_GEOMETRY(2001,8307,SDO_GEOM.SDO_POINT_TYPE(NVL(tmt.latitude,0),NVL(tmt.longitude,0),
...
'unit=M') )
				...
				WHERE 
					...
					AND TO_CHAR(THRP.TANGGAL_RENCANA,'YYYY-DD-MM') BETWEEN '{$start}' AND NVL ('{$end}', '{$start}') 
					AND ( 

<?php 
	...
	$sql = "
		...
		FROM (
			SELECT 
		    	...
		    	CASE WHEN status_tph = 'MANUAL' 
					AND (JARAK = '0.1' 
							OR JARAK > TOLERANSI_JARAK) 
				THEN 1 ELSE 0 END AS LOKASI_SALAH
			FROM (
				SELECT 
		    		...
		    		THP.ID_RENCANA,
					ROUND(tmt.latitude,8)    AS latitude_master_tph,
					ROUND(tmt.longitude,8)   AS longitude_master_tph,
					ROUND(thp.latitude,8)    AS latitude_input_bcc,
					ROUND(thp.longitude,8)   AS longitude_input_bcc,
		    		THP.STATUS_TPH,
		    		ROUND((CASE
                        WHEN
                            THP.STATUS_TPH = 'MANUAL' 
                        THEN
                            (CASE
                                WHEN 
                                    TMT.LATITUDE IS NULL OR TMT.LONGITUDE IS NULL THEN 0.1
                            ELSE
								(
									NVL(6387700,0) * ACOS((sin(TO_NUMBER(NVL(TRIM(TMT.LATITUDE),0)) / 57.29577951) *
									SIN(TO_NUMBER(NVL(TRIM(THP.LATITUDE),0)) / 57.29577951)) +
									(COS(TO_NUMBER(NVL(TRIM(TMT.LATITUDE),0)) / 57.29577951) * 
									COS(TO_NUMBER(NVL(TRIM(THP.LATITUDE),0)) / 57.29577951) *
									COS(TO_NUMBER(NVL(TRIM(THP.LONGITUDE),0)) / 57.29577951 - 
									TO_NUMBER(NVL(TRIM(TMT.LONGITUDE),0))/ 57.29577951)))
								)
							END)
                        ELSE
                            0.1
                    END),8) jarak,
		    		(SELECT NILAI FROM EBCC.T_PARAMETER WHERE KETERANGAN = 'RANGE') AS TOLERANSI_JARAK
				FROM EBCC.T_HEADER_RENCANA_PANEN THRP 
				INNER JOIN EBCC.T_DETAIL_RENCANA_PANEN TDRP ON THRP.ID_RENCANA = TDRP.ID_RENCANA 
				INNER JOIN EBCC.T_HASIL_PANEN THP ON TDRP.ID_RENCANA = THP.ID_RENCANA AND TDRP.NO_REKAP_BCC = THP.NO_REKAP_BCC 
				INNER JOIN EBCC.T_BLOK TB ON TDRP.ID_BA_AFD_BLOK = TB.ID_BA_AFD_BLOK 
				INNER JOIN EBCC.T_AFDELING TA ON TB.ID_BA_AFD = TA.ID_BA_AFD 
				INNER JOIN EBCC.T_BUSSINESSAREA TBA ON TA.ID_BA = TBA.ID_BA 
				LEFT JOIN EBCC.T_STATUS_TO_SAP_EBCC TSAP ON TBA.ID_CC = TSAP.COMP_CODE AND TBA.ID_BA = TSAP.PLANT AND THP.NO_BCC = TSAP.NO_BCC 
				...
				WHERE 
					...
					AND ( 
		        		(UPPER ('PEMANEN') = 'PEMANEN' AND (THRP.NIK_PEMANEN LIKE '%' || '' || '%' OR EBCC.F_GET_EMPNAME(THRP.NIK_PEMANEN) LIKE '%' || UPPER ('') || '%')) 
		        		OR (UPPER ('PEMANEN') = 'MANDOR' AND (THRP.NIK_MANDOR LIKE '%' || '' || '%' OR EBCC.F_GET_EMPNAME(THRP.NIK_MANDOR) LIKE '%' || UPPER ('') || '%')) 
	...
	if (empty($newarray)) {
		...
		foreach ($arr as $key => $val) {
			...
			$pdf->Cell(2.9, 0.5, 'LOKASI', 'T,L,R', 0, 'C', 1);
			$pdf->Cell(3, 1, 'NAMA ASLAP', 1, 0, 'C', 1);
			$pdf->Cell(3, 1, 'TTD', 1, 0, 'C', 1);
			$pdf->Cell(2.9, 0.5, 'JUMLAH EBCC', 'T,L,R', 0, 'C', 1);
			$pdf->Ln();
			
			$pdf->Cell(16.2 ,0.5, '', 0, 0, 'C', false);
			$pdf->Cell(2.9, 0.5, 'TIDAK SESUAI *', 'L,B,R', 0, 'C', 1);
			...
			$pdf->Cell(3, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(3.1, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(16, 0.5, ' * Lokasi Tidak Sesuai = EBCC yang lokasinya diinput manual (bukan scan) dan berada diluar radius', 'LR', 0, 'L');
			$pdf->Cell(5.9, 0.5, '', 'LR', 0, 'C');

			$pdf->Ln();
			$pdf->Cell(3, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(3.1, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(16, 0.5, '   master TPH', 'LR', 0, 'L');
			$pdf->Cell(5.9, 0.5, '', 'LR', 0, 'C');

			$pdf->Ln();
			$pdf->Cell(3, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(3.1, 0.5, '', 'LR', 0, 'C');
			$pdf->Cell(16, 0.5, '   Jika terdapat EBCC yang lokasi tidak sesuai, maka harus dilakukan Koreksi BCC', 'LR', 0, 'L');
			$pdf->Cell(5.9, 0.5, '', 'LR', 0, 'C');

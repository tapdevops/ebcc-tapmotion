	...
	$("#deliv_ticket").keyup(function(){
		...
		if(deliv_ticket==""){
			...
			$.ajax({
				type: "POST",
				url: "getTicket.php",
				data: "deliv_ticket="+ deliv_ticket+"&afdeling="+BA+afdeling + "&var_tgl=" + tgl_kirim ,
...
function fill(Value){
	...
	$('#tmp_tgl').val(split[0]);
	$('#tmp_blok').val(split[1]);
	$('#tmp_nik').val(split[2]);
	$('#tmp_tph').val(split[4]);
	$('#deliv_ticket').val(split[5]);
	$('#tmp_bcc').val(split[6]);
	$('#display').hide();
	var afdeling = $("#AFDlabel").val();
	var tgl_kirim = $("#datepicker").val();
    ...
    $.ajax({
        url: "getData.php",
        data: "countRow="+ countRow+"&deliv_ticket="+ deliv_ticket+"&afdeling="+BA+afdeling + "&var_tgl=" + tgl_kirim
      ...
      <table border="0" id="setbody2">
          ...
          <tr>
			...
			if($SessTIPE_ORDER == "EKSTERNAL"){
            	echo "<input name=\"No_PolisiLabel\" type=\"text\" id=\"No_PolisiLabel\" value=\"$sNO_POLISI\" style=\"width: 300px; height:25px; font-size:15px\" onchange=\"send(6)\" onblur=\"send(6)\" />"; 
			}
			else{
				echo "<input name=\"No_PolisiLabel\" type=\"text\" id=\"No_PolisiLabel\" value=\"$sNO_POLISI\" style=\"width: 300px; height:25px; font-size:15px\" onClick=\"javascript:showListNoPolisi();\" readonly='readonly'/>";
		...
		if($SessTIPE_ORDER == "INTERNAL"){
          ...
          <tr>
            ...
            <td><span style=\"font-size:16px\">
				<input name=\"Nama_Supir\" type=\"text\" id=\"Nama_Supir\" value=\"$Nama_Supir\" style=\"width: 300px; height:25px; font-size:15px\"  onClick='javascript:showListSupir(1);'/>            </span></td>
            <td>&nbsp;</td>
            <td>NIK Supir</td>
            <td>:</td>
		...
		<tr>
			<td><input name="tmp_tgl" type="text" id="tmp_tgl" value="" style="width:220px; height:20px; display:none;"/></td>
			<td><input name="tmp_blok" type="text" id="tmp_blok" value="" style="width:220px; height:20px; display:none;"/></td>
			<td><input name="tmp_nik" type="text" id="tmp_nik" value="" style="width:220px; height:20px; display:none;"/></td>
			<td><input name="tmp_tph" type="text" id="tmp_tph" value="" style="width:220px; height:20px; display:none;"/></td>
    ...
    <tr>
          ...
          <input name="s_Supir" type="text" id="s_Supir" value="<?=$NIK_Supir?>" style="display:none" onmousedown="return false"/>
			...
			<legend>BCC dikirim</legend>
				...
				<tr>
					<td align="right" width='755px'>Total Janjang</td>
					<td align="right" width='67px'>:</td>

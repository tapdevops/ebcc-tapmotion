...
function pick(symbol,row) {
    ...
    window.opener.document.inputBCC.NIK_TM1.value = symbol;
	window.opener.document.editNAB.s_TM1.value = symbol;
	window.close();
	}
}
function pickNAMA(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.inputBCC.Nama_TM1.value = symbol;
	window.close();
	}
}

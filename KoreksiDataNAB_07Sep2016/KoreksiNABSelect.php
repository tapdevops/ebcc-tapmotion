...
function fill(Value){
    ...
    $.ajax({
        url: "getData.php",
        data: "countRow="+ countRow+"&deliv_ticket="+ deliv_ticket+"&afdeling="+BA + "&var_tgl=" + tgl_kirim //Edited by Ardo, 15-02-2016 : Delivery Ticket diganti per BA
          ...
          <tr> <?php //method="post" action="KoreksiNABSelect.php" ?>
            ...
            <td style="font-size:16px">
				...
				while(oci_fetch($result_afd_by_ba)){
					...
					?> <option value="<? echo oci_result($result_afd_by_ba, "ID_AFD");?>" selected><?=oci_result($result_afd_by_ba, "ID_AFD")?></option><?php	
					} else {
					?> <option value="<? echo oci_result($result_afd_by_ba, "ID_AFD");?>"><?=oci_result($result_afd_by_ba, "ID_AFD")?></option><?php	
					}
				}
				?>
				</select>

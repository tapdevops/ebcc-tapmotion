...
$con = connect();
	...
	$query_emp = "Select distinct(SUBSTR(TE.ID_BA_AFD,1,4)) as BA, SUBSTR(TE.ID_BA_AFD,5,1) as AFD,
						(NIK_PEMANEN) as NIK, REPLACE(TE.EMP_NAME,'''','') as EMP_NAME,
						 JOB_CODE from T_HEADER_RENCANA_PANEN THRP 
                         LEFT JOIN T_DETAIL_RENCANA_PANEN TDRP ON (THRP.ID_RENCANA = TDRP.ID_RENCANA)
                         LEFT JOIN T_EMPLOYEE TE ON (TE.NIK = THRP.NIK_PEMANEN)
                         WHERE TANGGAL_RENCANA = TO_DATE('$tgl_panen', 'MM/DD/YYYY HH24:MI:SS') AND LUASAN_PANEN = '0' 
	...
	if ($numRow[0]==0) {
		...
		if($_POST) {
			...
			//}else{
				...
				//print_r($arr);die();
					$ar['EMP_NAME'] = str_replace("'", '', $ar['EMP_NAME']);
					?>
					<tr>
  ...
  FROM (  SELECT BA,
            ...
            FROM (  SELECT SUBSTR (TE.ID_BA_AFD, 1, 4) AS BA,
                           SUBSTR (TE.ID_BA_AFD, 5, 1) AS AFD,
                           (NIK_PEMANEN) AS NIK,
                           REPLACE (TE.EMP_NAME, '''', '') AS EMP_NAME,
                           JOB_CODE,
                           TO_CHAR (tanggal_rencana, 'MM/DD/YYYY')
                              AS tanggal_rencana,
                           tb.id_blok,
                           tdrp.ID_BA_AFD_BLOK,
                         ...
                         LEFT JOIN T_EMPLOYEE TE ON (TE.NIK = THRP.NIK_PEMANEN)
                         WHERE TANGGAL_RENCANA = TO_DATE('$tgl_panen', 'MM/DD/YYYY HH24:MI:SS') AND LUASAN_PANEN = '0' 
						 and TDRP.ID_BA_AFD_BLOK like '$afdeling%' and TE.INACTIVE_DATE is NULL
						 and THRP.NIK_PEMANEN = '".$arr['NIK']."'
						 order by (SUBSTR(TE.ID_BA_AFD,1,4)), SUBSTR(TE.ID_BA_AFD,5,1), TE.JOB_CODE, EMP_NAME) GROUP BY BA,
                 AFD,
                 tanggal_rencana,
                 NIK,
                 EMP_NAME,
                 id_blok,
                 ID_BA_AFD_BLOK)
 WHERE luasan_panen = 0 order by ba, afd, tanggal_rencana, emp_name";
			//echo $q_emp."<br>";
			$sql_emp = oci_parse($con,$q_emp);
			oci_execute($sql_emp, OCI_DEFAULT);
			$re = 0;
			$jml_re = 0;
			while ($aa=oci_fetch_array($sql_emp)){
				$jml_re++;
				$query_cek_export = "
				SELECT * FROM T_STATUS_TO_SAP_EBCC WHERE 
				TANGGAL = '".date('d.m.Y',strtotime($aa['TANGGAL_RENCANA']))."' AND 
				NIK_PEMANEN = '".$aa['NIK']."' AND 
				BLOCK = '".$aa['ID_BLOK']."' AND ((EXPORT_STATUS IS NOT NULL AND EXPORT_TIMESTAMP IS NOT NULL)
			...
			if($jml_re!=$re){
				$arr['EMP_NAME'] = str_replace("'", '', $arr['EMP_NAME']);
				?>
				<tr>

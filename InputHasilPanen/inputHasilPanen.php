...
<script type="text/javascript">
	...
	function getTPH(t) {
		var existhide = $('#hideexist').val();
		if (existhide > 0) {
			var id = t.id;
			var tph = id.replace('blok', 'tphnew');
			...
			$.ajax({
				...
				success: function(data) {
					if (data.total > 0) {
						$('#' + tph).html(data.output);
					} else {
						alert('Tidak ada TPH yang terdaftar pada blok');
					}
				}
			});
		} else {
		...
		$("#afdeling").change(function() {
			...
			$.ajax({
				...
				success: function(msg) {
					...
					//di <select id=kota>
					for (j = 1; j <= i; j++) {
						$("#blok" + j).html(msg.output);
						$('#hideexist').val(msg.exist);
						if (msg.exist > 0) {
							$('#tph-new-chosen').show();
							$('#tph-old-chosen').hide();
						} else {
					...
					success  : function(data) {
						...
						if(hasil[0] == '1') {
							...
							for (i = 1; i <= jmlrow; i++) {
								...
								if ($('#hideexist').val() > 0) {
									document.getElementById("tphnew" + i).value = "";
								} else {
									document.getElementById("tphold" + i).value = "";
								}
	...
	function validateInput(elem) {
		...
		for (i = 1; i <= jmlrow; i++) {
			...
			if ($('#hideexist').val() > 0) {
				v_tph = document.getElementById("tphnew" + i).value;
			} else {
				v_tph = document.getElementById("tphold" + i).value;
			}
			...
			<th height="197" scope="row" align="center">
				...
				<div style="overflow:scroll; width:1200px" id='page_hasil'>
					<fieldset>
						...
						<input type="hidden" id="hideexist">
						<table id="hasil_panen" border="0" cellpadding="1" cellspacing="1" style="table-layout:fixed; overflow-x:scroll">
							...
							<tr>
								...
								<td>
									<div id="tph-new-chosen">
										<select name="tphnew1" id="tphnew1">
											<option value='0' selected="selected">--select--</option>
										</select>
									</div>
									<div id="tph-old-chosen">
										<input style="width:34px;" maxlength="3" type='text' name='tphold1' id='tphold1' value='' onkeypress="return isNumber(event)">
									</div>
								</td>
...
<script language="JavaScript" type="text/JavaScript">
	...
	$("#add_row").click(function () {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				//for (j = 1; j <= i; j++){
					$('#hideexist').val(msg.exist);
					$("#blok" + i).html(msg.output);
					if (msg.exist > 0) {
						//$('#tph' + j).html(msg.exist);
						$('#tph-new-chosen').show();
						$('#tph-old-chosen').hide();
					} else {
						$('#tph-old-chosen').show();
	...
	function addRowToTable() {
		...
		sel.onblur = function() {
			...
			var i = document.getElementById('countRow').value; //added by NB 04.07.2014
			if ($('#hideexist').val() > 0) {
				var v_tph = document.getElementById('tphnew' + iteration).value;
			} else {
				var v_tph = document.getElementById('tphold' + iteration).value;
			}
		...
		if ($('#hideexist').val() > 0) {
			var tpel = document.createElement('select');
			tpel.name = 'tphnew' + iteration;
			tpel.id = 'tphnew' + iteration;
			tpel.options[0] = new Option('--select--', '0');
		} else {
			var tpel = document.createElement('input');
			tpel.type = 'text';
			tpel.name = 'tphold' + iteration;
			tpel.id = 'tphold' + iteration;
			tpel.value = '';
			tpel.style.width='34px';
			tpel.onkeypress = isNumber;
		}
		cell2.appendChild(tpel);

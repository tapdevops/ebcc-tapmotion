				...
				if(!is_numeric($Qty[$x])){
					...
					if($plan_ID != ""){
						if($plan_ID <> $ID_RENCANA){	
							...
							if(count($ID_Kualitas) >= 1 && $ID_Kualitas[$x] != ''){
								...
								$query_ins_thk = "UPDATE T_HASILPANEN_KUALTAS set ID_RENCANA = '" . $new_id_rencana . "' 
								WHERE ID_BCC = '" . $No_BCC . "' AND ID_RENCANA = '" . $ID_RENCANA . "'";
								$result_THPK = num_rows($con, $query_ins_thk);commit($con);
								//echo $result_THPK;								
								if ($x+1 == 16){
									//====== delete thp old
									$query_del_thp_old = "DELETE T_HASIL_PANEN 
									WHERE NO_REKAP_BCC = '" . $No_Rekap . "' 
									AND ID_RENCANA = '" . $ID_RENCANA . "'
									AND NO_BCC = '" . $No_BCC . "'";											
									$res_del_thk_old = num_rows($con, $query_del_thp_old); commit($con);
									$res_del_thk_old = 1;
								}
								//echo $res_del_thk_old;die();
							}
							
							if (($result_THPK <> 0 && $result_THPK < 17) && $res_del_thk_old == 1){ 						
							...
							if($new_no_rekap == ""){
								...
								$roweffec_value_hasil_panen = 1;
								$result_ins_no_rekap = 1;	
							}
							
							if($x + 1 == 16 AND $oldrencana!=$ID_RENCANA){
								
								
								$sql_update_bcc_thp = "UPDATE T_HASIL_PANEN set 
								ID_RENCANA = '".$plan_ID."' WHERE NO_REKAP_BCC = '" . $No_Rekap . "' 
								AND NO_BCC = '" . $No_BCC . "' 
								AND ID_RENCANA = '" . $oldrencana . "'";
								$roweffec_value_hasil_panen = num_rows($con, $sql_update_bcc_thp);
								
								$sql_update_hasilpanen_kualitas = "UPDATE T_HASILPANEN_KUALTAS set 
								ID_RENCANA = '".$plan_ID."' WHERE ID_BCC = '" . $No_Rekap . "'
								AND ID_RENCANA = '" . $oldrencana . "'";
								$roweffec_value_hasilpanen_kualitas = num_rows($con, $sql_update_hasilpanen_kualitas);
						...
						if($cek_id_rencana_THRP == ""){
							...
							'" . $nik_krani_buah . "', '" . $NIK_Pemanen . "', 'NO')";
							$result_insertemp = num_rows($con, $query_insertemp);
							print_r("asdasd".$result_insertemp);
							}else{
							$result_insertemp = 1;
						}
						...
						AND ID_KUALITAS = '".$ID_Kualitas[$x]."'";					
						$res_del_thk_old = num_rows($con, $query_del_thk_old);
						print_r("x = ".$x." Planid = ".$ID_Kualitas[$x]."<br>");

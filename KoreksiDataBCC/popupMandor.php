...
function pick(symbol,row) {
    ...
    window.opener.document.FormEditBCC.NIK_Mandor.value = symbol;
	window.close();
	}
}
function pickNAMA(symbol,row) {
	if (window.opener && !window.opener.closed) {
    window.opener.document.FormEditBCC.Nama_Mandorlabel.value = symbol;
	window.close();
	}
}
...
if ($numRow[0]==0) {
	...
	if($_POST) {
		...
		$query_emp = "select TJ.ID_BA as BA, 
					 ...
					 left join T_EMPLOYEE TE on TE.ID_JOBAUTHORITY = TJ.ID_JOBAUTHORITY
					 where AUTHORITY = '7' AND (" . $id_ba . ") and INACTIVE_DATE is NULL
						AND (NIK LIKE '%$search%' OR EMP_NAME LIKE '%$search%')
						order by TJ.ID_BA, SUBSTR(TE.ID_BA_AFD, 5,1), TE.JOB_CODE, EMP_NAME";
		$sql = oci_parse($con,$query_emp);
		oci_execute($sql, OCI_DEFAULT);
		$results=array();

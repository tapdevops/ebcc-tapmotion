	...
	if($NIK_Pemanen == "" || $roweffec_BCC == "" || $No_BCC	== ""){
		...
		for($x=0; $x < 16; $x++){
			...
			//cek id rencana
				...
				if($plan_ID != ""){
					...
					if ($cek_id_rencana_THP == ""){
						...
						if(count($ID_Kualitas) >= 1 && $ID_Kualitas[$x] != ''){
							...
							if ($x+1 == 16){
								//====== delete thp old
								$query_del_thp_old = "UPDATE T_HASIL_PANEN set ID_RENCANA = '" . $new_id_rencana . "'
														WHERE NO_REKAP_BCC = '" . $No_Rekap . "' 
														AND ID_RENCANA = '" . $ID_RENCANA . "'
														AND NO_BCC = '" . $No_BCC . "'";											
								$res_del_thk_old = num_rows($con, $query_del_thp_old);
								$res_del_thk_old = 1;
							}
				...
				//echo $result_THPK . " - " . $res_del_thk_old . " - " . $result_THP . " - " . $roweffec_change_data;die();
				if ( $new_id_rencana != ''){

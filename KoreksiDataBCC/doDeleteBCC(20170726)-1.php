<?php
	...
	if(isset($_SESSION['NIK']) && isset($_SESSION['LoginName'])){
		...
		&& isset($_REQUEST['nomor_ba']) && isset($_REQUEST['tanggal_ba']) && isset($_REQUEST['alasan'])){
			...
			/*$query_search_dthrp_avail = "SELECT * FROM DEL_T_HEADER_RENCANA_PANEN 
									WHERE ID_RENCANA = '" . $ID_RENCANA . "'";
									$result_search_dtrhp_avail = oci_parse($con, $query_search_dthrp_avail);
									oci_execute($result_search_dtrhp_avail, OCI_DEFAULT);
									oci_fetch($result_search_dtrhp_avail);
									$cek_result_search_dtrhp_avail = oci_result($result_search_dtrhp_avail, "ID_RENCANA");
			if($cek_result_search_dtrhp_avail == ''){*/

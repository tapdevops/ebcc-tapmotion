...
ORDER BY AFD_PEMANEN asc, nama_pemanen asc, nik_pemanen asc, id_blok asc, luasan_panen desc, nik_gandeng
		...
		$sql_Download_Crop_Harv = "
         ...
         thrp.nik_mandor,
         ebcc.f_get_empname@proddb_link (thrp.nik_mandor) nama_mandor,
         thrp.nik_pemanen,
         ebcc.f_get_idafd_nik@proddb_link (thrp.nik_pemanen) afd_pemanen,
         ebcc.f_get_empname@proddb_link (thrp.nik_pemanen) nama_pemanen,
         thp.no_bcc,
         thp.no_tph,
         thp.kode_delivery_ticket,
         tb.id_blok,
         ...
         NULL jam_kerja,
         thp.status_tph,
         CASE WHEN thp.status_tph = 'MANUAL' THEN 'Y' ELSE '' END
            status_input_manual,
         CASE WHEN thp.status_tph = 'WEBSITE' THEN 'Y' ELSE '' END
            status_koreksi_lokasi,
         (SELECT nilai
            FROM ebcc.t_parameter@proddb_link
           WHERE keterangan = 'RANGE')
            AS toleransi_jarak,
         (CASE
             WHEN thp.status_tph = 'MANUAL'
             THEN
                (CASE
                    WHEN tmt.latitude IS NULL OR tmt.longitude IS NULL
                    THEN
                       0.1
                    ELSE
                       ROUND(SDO_GEOM.sdo_distance (
                                ...
                                SDO_GEOM.sdo_geometry (
                                   2001,
                                   8307,
                                   sdo_point_type (NVL (thp.latitude, 0),
                                                   NVL (thp.longitude, 0),
                                                   NULL),
                                   NULL,
                                   NULL),
          ...
          END)
            jarak,
         thp.status_lokasi,
         NVL (ebcc.F_GET_HASIL_PANEN_BUNCH@proddb_link (tba.id_ba,
                                            thp.no_rekap_bcc,
                                            thp.no_bcc,
                                            'BUNCH_HARVEST'), 0)
            AS TBS,
         NVL (
            ebcc.F_GET_HASIL_PANEN_BRDX@proddb_link (thrp.id_rencana,
                                         thp.no_rekap_bcc,
                                         thp.no_bcc),
            0)
            AS BRD,
         NVL (ebcc.F_GET_HASIL_PANEN_NUMBERX@proddb_link (thrp.id_rencana,
                                              thp.no_rekap_bcc,
                                              thp.no_bcc,
                                              1), 0)
            AS BM,
         ...
         NVL (ebcc.F_GET_HASIL_PANEN_NUMBERX@proddb_link (thrp.id_rencana,
            ...
            AS ORR,
         NULL kode_absen,
         CASE WHEN ta.ID_BA <> ta2.ID_BA THEN 'CINT_' || ta.ID_BA ELSE NULL END
            customer,
         tb.blok_name,
         tdrp.no_rekap_bcc,
         thrp.id_rencana
    FROM ebcc.t_header_rencana_panen@proddb_link thrp
         INNER JOIN ebcc.T_EMPLOYEE@proddb_link te
            ON thrp.NIK_PEMANEN = te.NIK
         INNER JOIN ebcc.T_AFDELING@proddb_link ta2
         ...
         INNER JOIN ebcc.t_hasil_panen@proddb_link thp
            ON tdrp.id_rencana = thp.id_rencana
               AND tdrp.no_rekap_bcc = thp.no_rekap_bcc
         INNER JOIN ebcc.t_blok@proddb_link tb
            ON tdrp.id_ba_afd_blok = tb.id_ba_afd_blok
         INNER JOIN ebcc.t_afdeling@proddb_link ta
            ON tb.id_ba_afd = ta.id_ba_afd
         INNER JOIN ebcc.t_bussinessarea@proddb_link tba
            ON tba.id_ba = ta.id_ba
         INNER JOIN ebcc.t_companycode@proddb_link tc
            ON tba.id_cc = tc.id_cc
         LEFT JOIN tap_dw.tm_tph tmt
            ON tb.id_ba_afd_blok = tmt.werks || tmt.afd_code || tmt.block_code
               AND thp.no_tph = tmt.no_tph
   WHERE     tc.id_cc = '$ID_CC'
         AND tba.id_ba = '$ID_BA'
         AND ta.id_afd = NVL (DECODE ('$sesAfdeling', 'ALL', null, '$sesAfdeling'), ta.id_afd)
         AND thrp.nik_mandor =
               NVL (
                  DECODE ('".$_POST["NIKMandor"]."', 'ALL', null, '".$_POST["NIKMandor"]."'),
                  thrp.nik_mandor)
         AND TO_CHAR (thrp.tanggal_rencana, 'YYYY-MM-DD') BETWEEN '$date1' and nvl ('', '$date1')
ORDER BY tgl_panen,
         thrp.nik_mandor,
		...
		$result = oci_parse($cons, $sql_Download_Crop_Harv);
        ...
        <table cellspacing="0" class="scroll">
				...
				for ($z = 0; $z < $roweffec; $z++) {
					if (!empty($bcc_export[$z]['STATUS_TPH']) && !empty($bcc_export[$z]['STATUS_DETIC']) && $bcc_export[$z]['STATUS_TPH'] == 'MANUAL') {
						if ($bcc_export[$z]['JARAK'] == '.1') {
							$jarak = $bcc_export[$z]['JARAK'] + $bcc_export[$z]['TOLERANSI_JARAK'];
						} else {
							$jarak = $bcc_export[$z]['JARAK'];
						}
				...
				for ($counter = 0; $counter < $roweffec; $counter++){
					...
					if (!empty($bcc_export[$counter]['STATUS_TPH']) && !empty($bcc_export[$counter]['STATUS_DETIC']) && $bcc_export[$counter]['STATUS_TPH'] == 'MANUAL') {
						if ($bcc_export[$counter]['JARAK'] == '.1') {
							$jarak = $bcc_export[$counter]['JARAK'] + $bcc_export[$counter]['TOLERANSI_JARAK'];
						} else {
							$jarak = $bcc_export[$counter]['JARAK'];
						}
					...
					if($counter==0){
						...
						$cek_status_lokasi = "
							...
							and to_char(tanggal_rencana, 'YYYY-MM-DD') = '2018-04-12'
						";
						if (!empty($bcc_export[$counter]['STATUS_TPH']) && !empty($bcc_export[$counter]['STATUS_DETIC']) && $bcc_export[$counter]['STATUS_TPH'] == 'MANUAL') {
							if ($bcc_export[$counter]['JARAK'] == '.1') {
								$jarak = $bcc_export[$counter]['JARAK'] + $bcc_export[$counter]['TOLERANSI_JARAK'];
						...
						if($bcc_export[$counter]['NIK_KARYAWAN']!=$bcc_export[$counter-1]['NIK_KARYAWAN']){
							...
							if (!empty($bcc_export[$counter-1]['STATUS_TPH']) && !empty($bcc_export[$counter-1]['STATUS_DETIC']) && $bcc_export[$counter-1]['STATUS_TPH'] == 'MANUAL') {
								if ($bcc_export[$counter-1]['JARAK'] == '.1') {
									$jarak = $bcc_export[$counter-1]['JARAK'] + $bcc_export[$counter-1]['TOLERANSI_JARAK'];
								} else {
									$jarak = $bcc_export[$counter-1]['JARAK'];
								}
							...
							<tr>
								...
								<td width="70" align="center">
									<?php 
										$cek_status_lokasi = "
											SELECT thp.status_lokasi from t_header_rencana_panen thrp
											left join t_hasil_panen thp on thp.id_rencana = thrp.id_rencana
											where nik_pemanen = '{$bcc_export[$counter-1]['NIK_KARYAWAN']}' and to_char(tanggal_rencana, 'YYYY-MM-DD') = '{$date1}' AND thp.STATUS_TPH = 'MANUAL' AND thp.STATUS_LOKASI IS NULL";

										$result_status_lokasi = oci_parse($con, $cek_status_lokasi);
										oci_execute($result_status_lokasi, OCI_DEFAULT);
										$result_status_count = oci_fetch_all($result_status_lokasi, $result_record_status_lokasi, 0, -1, OCI_FETCHSTATEMENT_BY_ROW);

										echo ($result_status_count > 0) ? "" : "&#x2713;";
						...
						<td width="70" align="center">
							<?php 
								$cek_status_lokasi = "
									SELECT thp.status_lokasi from t_header_rencana_panen thrp
									left join t_hasil_panen thp on thp.id_rencana = thrp.id_rencana
									where nik_pemanen = '{$bcc_export[$counter]['NIK_KARYAWAN']}' and to_char(tanggal_rencana, 'YYYY-MM-DD') = '{$date1}' AND thp.STATUS_TPH = 'MANUAL' AND thp.STATUS_LOKASI IS NULL";

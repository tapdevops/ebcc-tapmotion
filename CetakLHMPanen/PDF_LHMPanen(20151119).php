...
<?php
	...
	*/
	...
	{//$totalBlok=0;
		...
		if(($ctr==$pagebreak || $NIKMandor[$j]!==$NIKMandor[$j+1] || $tglPanen[$j]!==$tglPanen[$j+1]) && $j+1<$i)
		{
			...
			if($NIKMandor[$j]!==$NIKMandor[$j+1] || $tglPanen[$j]!==$tglPanen[$j+1])
			{
				...
				$pdf->Cell(2,0.5,'Hasil Panen',1,0,'C');
				$pdf->Cell(7.7,0.5,'Pinalty',1,0,'C');
				$pdf->Cell(1,0.5,'',1,0,true);
				$pdf->Cell(1.2,0.5,'',1,0,true);
				$pdf->Ln();
				...
				$sql_cetak_panen = "	
							 ...
							 null jam_kerja,
								 ...
								 NVL (F_GET_HASIL_PANEN_BUNCH (tba.id_ba, thp.no_rekap_bcc, thp.no_bcc, 'BUNCH_PAID'), 0) as TBS,
								 ...
								 NVL (F_GET_HASIL_PANEN_NUMBER (thp.no_rekap_bcc, thp.no_bcc, 15), 0) JK, 
				...
				while (($data1 = oci_fetch_array($stid1, OCI_ASSOC))) 
				{
					...
					$pdf->Cell(3.1,0.5,$data1["NIK_PEMANEN"],1,0,'L');
					$pdf->Cell(4,0.5,$data1["NAMA_PEMANEN"],1,0,'L');
					$pdf->Cell(4,0.5,'',1,0,'C');
					$pdf->Cell(1,0.5,'',1,0,'C');
					$pdf->Cell(1,0.5,'',1,0,'C');
				...
				$pdf->Ln();
				$pdf->Cell(4,2.3,'',1,0,'L');
				$pdf->Cell(9.5,2.3,'',1,0,'C');
				$pdf->Cell(4,2.3,'',1,0,'C');
				...
				$pdf->Cell(5,0.3,'BS : Buah Sakit','R',0,'L');$pdf->Ln();
				$pdf->Cell(4,2,'',0,0,'L');
				$pdf->Cell(9.5,2,'',0,0,'C');
				$pdf->Cell(4,2,'',0,0,'C');
				$pdf->Cell(4.5,0.3,'BT : Buah Tinggal di Pokok',0,0,'L');
				$pdf->Cell(5,0.3,'BA : Buah Aborsi','R',0,'L');
	...
	while (($data2 = oci_fetch_array($stid2, OCI_ASSOC))) 
	{
		...
		$pdf->Cell(3.1,0.5,$data2["NIK_PEMANEN"],1,0,'L');
		$pdf->Cell(4,0.5,$data2["NAMA_PEMANEN"],1,0,'L');
		$pdf->Cell(4,0.5,'',1,0,'C');
		$pdf->Cell(1,0.5,'',1,0,'C');
		$pdf->Cell(1,0.5,'',1,0,'C');

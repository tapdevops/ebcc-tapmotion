...
<?php
	...
	while (($data = oci_fetch_array($stid, OCI_ASSOC))) 
	{
		...
		$cell[$i][22] = "";
		$cell[$i][23] = $data["BA"];
		$cell[$i][24] = $data["AFD_PEMANEN"];
		$cell[$i][25] = $data["CUSTOMER"];
	...
	for ($j=0;$j<$i;$j++)
	{
		...
		$page_header = 1;
		if(($NIKMandor[$j]!==$NIKMandor[$j+1] || $tglPanen[$j]!==$tglPanen[$j+1]) && $j+1<=$i)
		{
			...
			for($x=$start_lampiran;$x<=$j;$x++){
				...
				if($ctr==$pagebreak && $j+1<=$i){
					...
					$pdf->Cell(3,0.4,'Page',0,0,'L');
					$pdf->Cell(0.5,0.4,':',0,0,'L');
					$pdf->Cell(11,0.4,$page_lampiran.' dari '.ceil(($j-$start_lampiran+$totalBlok)/20),'R',0,'L');
					$pdf->Ln();
				...
				if($x==0){
					...
					&& $cell[$x][16]==$cell[$x+1][16]
					&& $cell[$x][17]==$cell[$x+1][17]
					&& $cell[$x][18]==$cell[$x+1][18]
					&& $cell[$x][19]==$cell[$x+1][19]
					&& $cell[$x][20]==$cell[$x+1][20]
				...
				$pdf->Cell(1,0.5,$cell[$x][24],1,0,'C',$fill_set);
				...
				$pdf->Cell(0.8,0.5,$cell[$x][7],1,0,'C',$fill_set);
				$pdf->Cell(1,0.5,$cell[$x][8],1,0,'C',$fill_set);
				$pdf->Cell(1,0.5,$cell[$x][9],1,0,'C',$fill_set);
				$pdf->Cell(0.7,0.5,$cell[$x][10],1,0,'C',$fill_set);
				...
				$pdf->Cell(1.3,0.5,$cell[$x][25],1,0,'C',$fill_set);
				$pdf->Ln();
			}
			...
			if(($ctr + $totalBlok) > $pagebreak)
			{
				...
				$pdf->Cell(3,0.4,'Page',0,0,'L');
				$pdf->Cell(0.5,0.4,':',0,0,'L');
				$pdf->Cell(11,0.4,($page_lampiran+1).' dari '.ceil(($j-$start_lampiran+$totalBlok)/20),'R',0,'L');
				$pdf->Ln();
				...
				//bagian untuk memasukkan keterangan tabel
				$pdf->SetFont('Arial','B',8);
				$pdf->Cell(0.6,0.5,'No.',1,0,'C',true);
				$pdf->Cell(1,0.5,'AFD',1,0,'C',true);
				$pdf->Cell(2.2,0.5,'NIK',1,0,'C',true);
				$pdf->Cell(4.5,0.5,'Nama Karyawan',1,0,'C',true);
				$pdf->Cell(3,0.5,'OPH/BCC',1,0,'C',true);
				$pdf->Cell(0.8,0.5,'TPH',1,0,'C',true);
				$pdf->Cell(0.8,0.5,'Blok',1,0,'C');
				$pdf->Cell(0.8,0.5,'Desc',1,0,'C');
				$pdf->Cell(0.8,0.5,'HA',1,0,'C');
				$pdf->Cell(0.8,0.5,'Jam',1,0,'C');
				$pdf->Cell(2,0.5,'Hasil Panen',1,0,'C');
				$pdf->Cell(8.4,0.5,'Pinalty',1,0,'C',true);
				$pdf->Cell(1,0.5,'Kode',1,0,'C',true);
				$pdf->Cell(1.3,0.5,'Cust',1,0,'C',true);
				$pdf->Ln();
				$pdf->Cell(0.6,0.5,'',1,0,'C',true);
				$pdf->Cell(1,0.5,'KARY',1,0,'C',true);
				$pdf->Cell(2.2,0.5,'',1,0,'C',true);
				$pdf->Cell(4.5,0.5,'',1,0,'C',true);
				$pdf->Cell(3,0.5,'',1,0,'C',true);
				$pdf->Cell(0.8,0.5,'',1,0,'C',true);
				...
				$pdf->Cell(0.8,0.5,'',1,0,'C',true);
				$pdf->Cell(0.8,0.5,'Kerja',1,0,'C');
				$pdf->Cell(1,0.5,'TBS',1,0,'C',true);
				$pdf->Cell(1,0.5,'BRD',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BM',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BK',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'TP',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BB',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'JK',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BA',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BT',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BL',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'PB',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'AB',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'SF',1,0,'C',true);
				$pdf->Cell(0.7,0.5,'BS',1,0,'C',true);
				$pdf->Cell(1,0.5,'Absen',1,0,'C',true);
				$pdf->Cell(1.3,0.5,'',1,0,'C',true);
				$pdf->Ln();
				$pdf->Cell(28,0.1,'',1,0,'C');
				$pdf->Ln();
			...
			for($l=0;$l<$totalBlok;$l++)
			{
				...
				$pdf->Cell(0.8,0.5,'',1,0,'C');
				$pdf->Cell(0.8,0.5,$luasan[$l],1,0,'C');
				$pdf->Cell(0.8,0.5,'',1,0,'C');
				$pdf->Cell(1,0.5,$totalTBS[$l],1,0,'C');
				$pdf->Cell(1,0.5,$totalBRD[$l],1,0,'C');
				$pdf->Cell(10.7,0.5,'',1,0,'C');
				$pdf->Ln();

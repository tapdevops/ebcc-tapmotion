...
thp.status_lokasi, tmt.latitude m_latitude, tmt.longitude m_longitude, thp.latitude tph_latitude, thp.longitude tph_longitude", "INNER JOIN t_companycode tc ON tba.id_cc = tc.id_cc
    LEFT JOIN tap_dw.tm_tph@proddw_link tmt ON tb.id_ba_afd_blok = tmt.werks || tmt.afd_code || tmt.block_code AND thp.no_tph = tmt.no_tph"), $sql_value);
//echo $sql_value; echo '<br /><br /><Br />';
    ...
    LEFT JOIN tap_dw.tm_tph@proddw_link tmt ON tb.id_ba_afd_blok = tmt.werks || tmt.afd_code || tmt.block_code AND thp.no_tph = tmt.no_tph", $sql_value);
//echo $sql_value; echo '<br /><br /><Br />';die();
    $head = "
    	SELECT 
id_cc,
comp_name,
...
from (
	...
	if($i==0){
		/*added by NB 20.08.2014*/
			...
			$pdf->Cell(4.4,0.5,'Nama Karyawana',1,0,'C');
			...
			$pdf->Cell(4.5,0.5,'Nama Karyawanb',1,0,'C');
	...
	for ($j=0;$j<$i;$j++)
	{
		...
		if(($NIKMandor[$j]!==$NIKMandor[$j+1] || $tglPanen[$j]!==$tglPanen[$j+1] || $IDAfd[$j]!==$IDAfd[$j+1]) && $j+1<=$i)
		{
            ...
            thp.latitude tph_latitude,
            thp.longitude tph_longitude,
	...
	order by   tgl_panen, thrp.nik_mandor,  nama_pemanen,  tdrp.no_rekap_bcc,  thrp.id_rencana, thp.no_bcc) header
				group by tgl_panen, afd_pemanen, nik_pemanen, nama_pemanen, id_blok, luasan_panen, customer, status_input_manual, status_koreksi_lokasi, m_latitude, m_longitude, tph_latitude, tph_longitude order by tgl_panen, nik_pemanen) detail
	GROUP BY afd_pemanen, tgl_panen, nik_pemanen, nama_pemanen, id_blok, customer, status_input_manual, status_koreksi_lokasi, m_latitude, m_longitude, tph_latitude, tph_longitude
	            ORDER BY tgl_panen, nik_pemanen)
	group by afd_pemanen, nik_pemanen, nama_pemanen, customer, status_input_manual, status_koreksi_lokasi, m_latitude, m_longitude, tph_latitude, tph_longitude order by afd_pemanen, nama_pemanen, nik_pemanen";
	//echo $sql_cetak_panen; echo '<br /><br />';
			/*$sql_cetak_panen = "
SELECT
    afd_pemanen,
    nik_pemanen,
...
ORDER BY
	...
	nik_pemanen
...
) GROUP BY afd_pemanen, nik_pemanen, nama_pemanen, customer order by nama_pemanen";*/
			...
			$pdf->Cell(3,0.5,'AFD KARY',1,0,'C');
			$pdf->Cell(3.1,0.5,'NIK',1,0,'C');
			$pdf->Cell(4.4,0.5,'Nama Karyawanc',1,0,'C');
			$pdf->Cell(1,0.5,'',1,0,'C',true);//
			$pdf->Cell(0.5,0.5,'',1,0,'C',true);//
			$pdf->Cell(1.1,0.5,'HA',1,0,'C');
			$pdf->Cell(1,0.5,'Jam',1,0,'C');
			...
			for ($clap = 0; $clap < $laporan_count; $clap++)
			{
				...
				if($jmlbaris > $pagebreak1){
					...
					$pdf->Cell(3,0.5,'AFD KARY',1,0,'C');
					$pdf->Cell(3.1,0.5,'NIK',1,0,'C');
					$pdf->Cell(4.4,0.5,'Nama Karyawand',1,0,'C');
					$pdf->Cell(1,0.5,'',1,0,'C',true);//
					$pdf->Cell(0.5,0.5,'',1,0,'C',true);//
					$pdf->Cell(1.1,0.5,'HA',1,0,'C');
					$pdf->Cell(1,0.5,'Jam',1,0,'C');
				...
				$pdf->Cell(1,0.5,'',1,0,'C',true);//
				$pdf->Cell(0.5,0.5,'',1,0,'C',true);//
				//echo $laporan_result[$clap]["NIK_PEMANEN"]; echo '<br />';
				...
				$query_luasan_panen = "
		 			...
		 			FROM (
		 				...
		 				where     tc.id_cc = '$id_cc'
		 					...
		 					and thrp.nik_mandor = nvl(decode('$NIKMandor[$j]', 'ALL', null, '$NIKMandor[$j]'), thrp.nik_mandor)
		 					and TO_CHAR (thrp.tanggal_rencana, 'YYYY-MM-DD') = '$tgl'
		 					AND   thrp.nik_pemanen = '{$laporan_result[$clap]["NIK_PEMANEN"]}'
		 				ORDER BY
		 					id_blok,
		 					luasan_panen DESC
		 			)
		 		...
		 		$con_luasan = connect();
		 		$stid_luasan = oci_parse($con_luasan, $query_luasan_panen);
		 		oci_execute($stid_luasan);

		 		$cell_luasan = array();
		 		$z = 0;
		 		while (($data = oci_fetch_array($stid_luasan, OCI_ASSOC))) {
		 			$cell_luasan[$z]['blok'] = $data["ID_BLOK"];
		 			$cell_luasan[$z]['luasan_panen'] = $data["LUASAN_PANEN"];
		 			$z++;
		 		}
		 		$input = array_map("unserialize", array_unique(array_map("serialize", $cell_luasan)));

		 		$total_luasan_count = 0;
		 		foreach ($input as $row) {
		 			$total_luasan_count = $total_luasan_count + $row['luasan_panen'];
		 		}

				$pdf->Cell(1.1,0.5,number_format($total_luasan_count,2),1,0,'C');
				...
				//print_r ($laporan_result[$clap]['TPH_LATITUDE']);
				/*(
					CASE
						WHEN thp.status_tph = 'MANUAL' THEN (
							CASE
				...
				//print_r ($laporan_result[$clap]);

				$laporan_result[$clap]['STATUS_TPH'] = 'MANUAL';
				if ($laporan_result[$clap]['STATUS_TPH'] == 'MANUAL') {
					$laporan_result[$clap]['M_LATITUDE'] = '-2.9577816667';
					$laporan_result[$clap]['M_LONGITUDE'] = '112.31808';
					//echo $laporan_result[$clap]['M_LATITUDE']; echo '<br />';
					//echo $laporan_result[$clap]['M_LONGITUDE']; echo '<br />';
					//echo $laporan_result[$clap]['TPH_LATITUDE']; echo '<br />';
					//echo $laporan_result[$clap]['TPH_LONGITUDE']; echo '<br /><br />';
					if (empty($laporan_result[$clap]['M_LATITUDE']) && empty($laporan_result[$clap]['M_LONGITUDE'])) {
						$jarak = 0.1;
					} else {
						$jarak = get_meters_between_points($laporan_result[$clap]['M_LATITUDE'], $laporan_result[$clap]['M_LONGITUDE'], $laporan_result[$clap]['TPH_LATITUDE'], $laporan_result[$clap]['TPH_LONGITUDE']);
					}
				} else {
					$jarak = 0.1;
				}
				...
				#CASE WHEN status_tph = 'MANUAL' AND ( jarak = '0.1' OR status_lokasi <> '1' OR jarak > toleransi_jarak ) THEN 1 ELSE 0 END AS status_koreksi_lokasi,
				if ($laporan_result[$clap]['STATUS_TPH'] == 'MANUAL' AND ($jarak = '0.1' OR $laporan_result[$clap]['STAUTS_LOKASI'] != '1' OR $jarak > $laporan_result[$clap]['TOLERANSI_JARAK'])) {
					$laporan_result[$clap]['STATUS_KOREKSI_LOKASI'] = 1;
				} else {
					$laporan_result[$clap]['STATUS_KOREKSI_LOKASI'] = 0;
				}
			...
			$pdf->Cell(4.0,0.5,'Nama Karyawane',1,0,'C');
			$pdf->Cell(3,0.5,'OPH/BCC',1,0,'C');
			$pdf->Cell(0.8,0.5,'TPH',1,0,'C');
			$pdf->Cell(0.8,0.5,'Blok',1,0,'C');
			$pdf->Cell(0.8,0.5,'Desc',1,0,'C');
			...
			for($x=$start_lampiran;$x<=$j;$x++){
				...
				if($ctr==$pagebreak && $j+1<=$i){
					...
					$pdf->Cell(1,0.5,'AFD',1,0,'C');
					$pdf->Cell(2.2,0.5,'NIK',1,0,'C');
					$pdf->Cell(4,0.5,'Nama Karyawanf',1,0,'C');
					$pdf->Cell(3,0.5,'OPH/BCC',1,0,'C');
					$pdf->Cell(0.8,0.5,'TPH',1,0,'C');
					$pdf->Cell(0.8,0.5,'Blok',1,0,'C');
					$pdf->Cell(0.8,0.5,'Desc',1,0,'C');

    ...
    <th height="197" scope="row" align="center"><table width="937" border="0" id="setbody2">
      ...
      <form id="doFilter" name="doFilter" method="post" action="doFilter.php">
        <tr>
          ...
          <td  valign="top"><?php
		  //Mandor
        if (!is_null($subID_BA_Afd) and !is_null($sdate1)){
			$sql_MD = "  SELECT nik_mandor NIK_MANDOR, emp_name NAMA_MANDOR
							FROM ebcc.t_header_rencana_panen thrp
								 ...
								 LEFT JOIN ebcc.t_employee emp
									ON thrp.nik_mandor = emp.nik
						   WHERE ta.id_ba = '$subID_BA_Afd' AND ta.id_afd = NVL (DECODE ('$sesAfdeling', 'ALL', NULL, '$sesAfdeling'), ta.id_afd) AND TO_CHAR (thrp.tanggal_rencana, 'YYYY-MM-DD') BETWEEN '$sdate1' AND NVL ('$sdate2', '$sdate1')
						GROUP BY nik_mandor, emp_name";	
		}
		else {
			$sql_MD = "  SELECT ' ' NIK_MANDOR, ' ' NAMA_MANDOR
						FROM dual";
		}

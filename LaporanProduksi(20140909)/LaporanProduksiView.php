			...
			if(isset($_GET["submitvalue"]) == 1){
				$sql_t_hpk  = "SELECT 
					...
					FROM (
						...
						TDRP.LUASAN_PANEN,
						TDRP.NO_REKAP_BCC,
						F_GET_TBS_PROD (TDRP.ID_RENCANA,TDRP.NO_REKAP_BCC) TBS , 
						F_GET_BRD_PROD (TDRP.ID_RENCANA,TDRP.NO_REKAP_BCC) BRD
						FROM T_HEADER_RENCANA_PANEN THRP 
						INNER JOIN T_DETAIL_RENCANA_PANEN TDRP ON THRP.ID_RENCANA = TDRP.ID_RENCANA 
						...
						where 
					   tba.id_cc = '$subID_CC' and tba.ID_BA = '$sID_BA' 
						and ta.id_afd = nvl (decode ('$sAFD', 'ALL', null, '$sAFD'), ta.id_afd) 
						and tb.id_blok = nvl (decode ('$sID_BLOK', 'ALL', null, '$sID_BLOK'), tb.id_blok)
						and thrp.tanggal_rencana  between TO_DATE('$sdate1','RRRR-mm-dd') and TO_DATE(nvl ('$sdate2', '$sdate1'),'RRRR-mm-dd')
								 and thrp.nik_pemanen = nvl (decode ('$sNIK_Pemanen', 'ALL', null, '$sNIK_Pemanen'), thrp.nik_pemanen) 
						and thp.status_bcc = upper('DELIVERED') 
						...
						TDRP.LUASAN_PANEN,
						TDRP.NO_REKAP_BCC,
						F_GET_TBS_PROD (TDRP.ID_RENCANA,TDRP.NO_REKAP_BCC),
						F_GET_BRD_PROD (TDRP.ID_RENCANA,TDRP.NO_REKAP_BCC) ) 
					   group by tanggal, id_cc, id_ba, id_afd, id_blok, blok_name order by tanggal, id_cc, id_ba, id_afd, id_blok";
...
<table width="1151" height="390" border="0" align="center">
  ...
  <tr>
    <th height="197" scope="row" align="center"><table width="937" border="0" id="setbody2">
      <tr>
        <td colspan="3" align="left" valign="baseline"><span style="font:bold; font-size:20px; font-weight: bold;"><strong>LAPORAN PRODUKSI</strong></span></td>
		<td colspan="3" align="right"><a href="printXLS.php"><input type="submit" name="button" id="button" value="DOWNLOAD TO XLS" style="width:200px; height: 30px; font-size:16px; visibility:<?=$visisub?>" onclick="formSubmit(1)"/></a></td>
      </tr>
	  <tr>
      ...
      <tr>
		...
		<td width="100" valign="top" ><input type="text" name="date1" id="datepicker" class="box_field" onchange="this.form.submit();" <?php if(isset($_POST["date1"])){ echo "value='$_POST[date1]'"; }?>></td>
		</tr>
		<tr>
		...
		<td width="10" height="29" valign="top" >:</td>
		<td width="100" valign="top" ><input type="text" name="date2" id="datepicker2" class="box_field" onchange="this.form.submit();" <?php if(isset($_POST["date2"])){ echo "value='$_POST[date2]'"; }?>></td>
		</tr>
		</form>
...
?>
        ...
        <td valign="top" >:</td>
        <td colspan="3" valign="top"><form id="formtampilkan" name="formtampilkan" method="post" action="LaporanProduksiView.php?submitvalue=1">
...
echo "<table width=\"1100px\"  border=\"1\" bordercolor=\"#9CC346\">
          <tr bgcolor=\"#9CC346\" >
            ...
            <td width=\"200px\" colspan=\"2\" align=\"center\" style=\"font-size:14px\" id=\"bordertable\">Produksi</td>
          </tr>

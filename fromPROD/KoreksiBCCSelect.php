...
if($username == "") {
	...
	while(oci_fetch($result_t_BCC)) {
		...
		$STATUS_TPH = oci_result($result_t_BCC, "STATUS_TPH");
		$TOLERANSI_JARAK = oci_result($result_t_BCC, "TOLERANSI_JARAK");
	}

	$sql_m_LatLong = "
		SELECT latitude, longitude FROM TAP_DW.TM_TPH@PRODDW_LINK WHERE WERKS = '{$BA}' AND AFD_CODE = '{$AFD}' AND BLOCK_CODE = '{$ID_BLOK}' AND NO_TPH = '{$BLOK_TPH}'
	";
	//echo $sql_m_LatLong; echo '<br /><br />';
...
<?php
	...
	// condition by levi 2019 aug 22
	if(!$LAT_M_TPH || !$LONG_M_TPH || !$LAT_TPH || !$LONG_TPH){
		$jarak = 'Tidak dapat ditentukan';
	}else{
	...
	$(document).ready(function(){
		...
		$('#selectblok').change(function() {
			...
			$.ajax({
				...
				success: function(data) {
					if (data.total > 0) {
						$('#selecttph').html(data.output);
						$('#selecttphawal').val(etph);
					} else {
						if (data.new > 0) {
							$('#jarak-geotagging').show();
							$('#tph-terdekat').show();
							$('#new-tph').show();
							$('#old-tph').hide();
							alert('Geotagging untuk TPH ' + etph + ' tidak ada');
						} else {
							$('#jarak-geotagging').hide();
							$('#tph-terdekat').hide();
							$('#old-tph').show();
							$('#new-tph').hide();
						}
			...
			<table width="991" border="0" style="border:solid #556A29;">
				...
				<tr>
					...
					<td align="center">
						...
						echo "
							<script>
							console.log('$STATUS_LOKASI != \'1\' && $STATUS_TPH == \'MANUAL\' && ((int)str_replace(\'.\', \'\', (!is_numeric($jarak) ? 1000000:$jarak)) > $TOLERANSI_JARAK)');
							</script>
						";
						...
						<?php 
						// add by levi 2019 aug 22
						if(!$LAT_M_TPH || !$LONG_M_TPH){
							echo '<div style="background-color: #fbd597;color: #222222;padding: 5px; margin:20px 0px;">TPH '.$BLOK_TPH.' belum terregister koordinatnya</div>';
						}
						
						if(!$LAT_TPH || !$LONG_TPH){
							echo '<div style="background-color: #fbd597;color: #222222;padding: 5px;">Pencatatan hasil panen tidak ditemukan koordinatnya</div>';
						}
						?>
					</td>
				</tr>
						...
						<table id="tph-terdekat" class="tph-terdekat" width="100%" border="0" style="border:solid #556A29;">
							<thead style="border-bottom:3px solid #556A29;">
								<tr>
									...
									<th style="font-weight:bold; padding-top:10px; padding-bottom:10px; border-right: 2px solid #556A29;">Blok</th>
								...
								$sql = "
									SELECT werks, afd_code, blok_name, block_code, latitude, longitude, no_tph, jarak FROM (
										...
										FROM (
											...
											FROM TAP_DW.TM_TPH@PRODDW_LINK z
											--  FROM T_HASIL_PANEN z
											JOIN (
												SELECT 
													$LAT_TPH AS latpoint, $LONG_TPH AS longpoint,
									...
									LEFT JOIN t_afdeling ta ON ta.id_ba_afd = tb.id_ba_afd 
							...
							<tbody>
								<?php while ($row = oci_fetch_array($result_latlong, OCI_ASSOC+OCI_RETURN_NULLS)) : ?>
									...
									<tr>
										...
										<td style="text-align:center; padding-top:5px; padding-bottom:5px; border-bottom:2px solid #556A29; border-right:2px solid #556A29; padding-left:5px; padding-right:5px;"><?php echo $row['BLOCK_CODE']; ?> - <?php echo $row['BLOK_NAME']; ?></td>

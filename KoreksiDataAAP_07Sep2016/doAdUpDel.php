					...
					for($x=0; $x < $roweffec_DETAILGANDENG; $x++){
						...
						else
						{
							...
							if($roweffec_select_NIKExist > 0){
								$sql_check = "select * from t_detail_gandeng WHERE ID_RENCANA = '$ID_RENCANA' AND NIK_GANDENG = '$NIK_GANDENG[$x]'";
								$roweffec_check = select_data($con,$sql_check);
								echo $sql_check."<br>";
								
								if($roweffec_check > 0){
									$sql_value[$x] = "UPDATE t_detail_gandeng SET NIK_GANDENG = '$NewNIK_GANDENG[$x]' WHERE ID_RENCANA = '$ID_RENCANA' AND NIK_GANDENG = '$NIK_GANDENG[$x]'";
									$roweffec_value[$x] = num_rows($con,$sql_value[$x]);
								}
								else{
